...
Gunakan mode ini hanya untuk membuat atau memperbarui artefak module-blueprint persisten, termasuk manifest, status, dokumentasi bisnis dan arsitektur berbasis bukti, decision, contract, Mermaid `classDiagram` pada `02-backend-architecture.md`, kamus data pada `data/data-dictionary.md`, flowchart alur proses pada `flowcharts/**`, roadmap, dan artefak readiness. Folder `erd/` **RETIRED** dan tidak lagi dibuat; kontrak keluaran canonical ada pada `design-business-module/references/blueprint-output-contract.md`. Mode ini tidak pernah memberi wewenang implementasi aplikasi, perubahan controller/service/entity, migration atau tindakan database, perubahan dependency/package, deployment, maupun publikasi Git. Jangan mengarang keputusan bisnis atau mengubah source hanya agar sesuai dengan blueprint.
