...
| `RAD-OPEN-011` | Open question | Peran Quilvian mana yang memegang `RadSafetyRule : Approve`, `: Reject`, dan `: Deactivate` atas nama Komite Medis, dan siapa pemegang akunnya. Tanpa ini, `RAD-DEC-016` tidak dapat dijalankan walau kewenangannya sudah jelas — badan tidak dapat masuk ke sistem | Komite Medis, dijalankan Administrator | `open` | — | Konsekuensi langsung `RAD-DEC-016`; hak akses Quilvian melekat pada peran pengguna |
