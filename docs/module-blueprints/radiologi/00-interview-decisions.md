...
| Clinical governance owner | **Komite Medis** sebagai badan (`RAD-DEC-016`), dijalankan **Yoga Aji Pratama** sebagai pemegang akun (`RAD-DEC-017`, 2026-09-14) |
...
| `RAD-DEC-016` | Decision | **Penanggung jawab tata kelola klinis modul Radiologi adalah Komite Medis**, sebagai badan — bukan perorangan. Kewenangan mengesahkan aturan keselamatan, menolaknya, dan menghentikannya melekat pada badan itu. **Menutup pertanyaan kewenangan, belum menutup pelaksanaannya**: hak akses Quilvian melekat pada peran pengguna, sehingga Komite Medis tetap perlu menunjuk pemegang akun yang menjalankannya sehari-hari — lihat `RAD-OPEN-011` | Manajemen rumah sakit | `approved` | Yoga Aji Pratama, 2026-09-11 | Jawaban pemilik modul 2026-09-11 atas `RAD-REQ-002` butir 1 |
| ~~`RAD-OPEN-011`~~ | Open question | ~~Peran Quilvian mana yang memegang `RadSafetyRule : Approve`, `: Reject`, dan `: Deactivate` atas nama Komite Medis, dan siapa pemegang akunnya~~ **DITUTUP 2026-09-14 oleh `RAD-DEC-017`.** Pemegang akunnya **Yoga Aji Pratama** | Komite Medis, dijalankan Administrator | `closed` | Yoga Aji Pratama, 2026-09-14 | Ditutup `RAD-DEC-017` |
| `RAD-DEC-017` | Decision | **Pemegang akun tata kelola klinis Radiologi adalah Yoga Aji Pratama**, menjalankan kewenangan Komite Medis sehari-hari. Ia memegang `RadSafetyRule : Approve`, `: Reject`, dan `: Deactivate`. Penanda `RadReport : ActAsRadiologist` **tidak** melekat padanya, melainkan pada posisi **Dokter Radiologi** — keduanya kewenangan yang berbeda dan sengaja tidak digabung. Menutup `RAD-OPEN-011`, melaksanakan `RAD-DEC-016` | Komite Medis | `approved` | Yoga Aji Pratama, 2026-09-14 | Instruksi pemilik modul 2026-09-14; melaksanakan `RAD-DEC-016` |
| `RAD-OPEN-012` | Open question | **Siapa yang mengajukan aturan keselamatan**, karena `AC-13` melarang penyusun **dan** pengaju mengesahkan aturan yang sama. Dengan Yoga Aji Pratama sebagai satu-satunya pengesah, langkah *Ajukan* wajib dijalankan orang lain — menunjuk satu pengesah saja **tidak** membuat modul dapat berjalan | Komite Medis, dijalankan Administrator | `open` | — | Konsekuensi `RAD-DEC-017`; `RadSafetyPolicyService.ApproveAsync` menolak `actorUserId == SubmittedByUserId \|\| actorUserId == CreateBy` |
| `RAD-CONF-DEC-08` | Decision | Seluruh requirement PRD 1.0 memakai awalan `PRD-` — `PRD-FR-RAD-001`…`012` dan `PRD-AC-001`…`010`. ID blueprint **tidak disentuh** karena sudah dirujuk 28 task roadmap dan 15 laporan task backend. Awalan `BP-RAD-` dan `BR-RAD-` tidak berubah karena tidak pernah bentrok | Pemilik modul | `approved` | Yoga Aji Pratama, 2026-09-11 | Tinjauan konformansi PRD 2026-09-11; menutup `RAD-CONF-ISSUE-01` pada `05-prd-conformance-gap.md` |
| `RAD-CONF-DEC-09` | Decision | Pengesahan bertingkat hasil bacaan dan koreksi berversi **tetap berlaku dan tetap wajib**; tidak ada yang ditarik kembali. Ketika PRD 1.0 berselisih dengan keputusan terkunci, **keputusan terkunci yang menang** — PRD belum pernah melewati `/qv-grill` maupun `requirement-completeness-gate`. Tanda tangan elektronik hasil tetap di luar scope | Pemilik modul | `approved` | Yoga Aji Pratama, 2026-09-11 | Tinjauan konformansi PRD 2026-09-11; menutup `RAD-CONF-ISSUE-02`; menguatkan `RAD-DEC-003` dan `RAD-DEC-004` |
...
| 12 | 2026-09-14 | `RAD-DEC-017` menunjuk **Yoga Aji Pratama** sebagai pemegang akun tata kelola klinis, menjalankan kewenangan Komite Medis sehari-hari — menutup `RAD-OPEN-011`. Penanda `RadReport : ActAsRadiologist` ditetapkan melekat pada posisi **Dokter Radiologi**, sengaja terpisah dari kewenangan pengesahan aturan. `RAD-OPEN-012` dibuka: `AC-13` melarang penyusun **dan** pengaju mengesahkan aturan yang sama, sehingga menunjuk satu pengesah saja belum membuat modul dapat berjalan — langkah *Ajukan* wajib dijalankan orang lain. | `draft` |
| 11 | 2026-09-11 | `RAD-DEC-016` menunjuk **Komite Medis** sebagai penanggung jawab tata kelola klinis modul Radiologi, sebagai badan — menutup `RAD-OPEN-001`. `RAD-OPEN-011` dibuka: badan tidak dapat masuk ke sistem, sehingga pemegang akun pengesahnya masih perlu ditunjuk Komite Medis sendiri. | `draft` |
| 10 | 2026-09-11 | `RAD-CONF-DEC-08` dan `RAD-CONF-DEC-09` dikunci dari tinjauan konformansi PRD eksternal versi 1.0. Yang pertama menetapkan awalan `PRD-` bagi requirement PRD; yang kedua menegaskan pengesahan bertingkat dan koreksi berversi tetap berlaku, dan keputusan terkunci menang atas PRD. Rinciannya pada `05-prd-conformance-gap.md`. | `draft` |
