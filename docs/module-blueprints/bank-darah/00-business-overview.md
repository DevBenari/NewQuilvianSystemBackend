...
| `FAK-BD-005` | `Fact` | Bank Darah **sudah terdaftar** di registry kepemilikan modul dengan prefix `Bbk`, dan Lifecycle-nya **`ACTIVE`**. Semula belum terdaftar; pendaftaran ditutup 3 September 2026, aktivasi menyusul di hari yang sama. | — | terverifikasi | `docs/engineering/MODULE_OWNERSHIP_PREFIX_REGISTRY.md` commit `ed7fba8` (pendaftaran, Lifecycle `PLANNED`) lalu `8075784` (aktivasi `PLANNED` → `ACTIVE`). `ACTIVE` membuka wewenang implementasi entity `Bbk*`; eksekusi database di luar dev pemilik dan deployment tetap terpisah |
