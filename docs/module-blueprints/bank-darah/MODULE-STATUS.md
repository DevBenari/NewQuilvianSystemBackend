...
| Revision | `26` — tidak naik pada roadmap revisi 11; perpindahan pemilik kriteria dan pencatatan kebijakan verifikasi bukan perubahan material atas blueprint target |
...
### Per 12 September 2026 — sesudah roadmap revisi 11

| Urutan | Tindakan | Pemilik | Sifat |
| --- | --- | --- | --- |
| 1 | **Build manual `dotnet build` lalu lanjutkan `BE-BD-006`** — urutan pastinya di bagian 8 [laporan](task/report/backend/BE-BD-006.md) | **Pemilik** untuk build, database, dan commit; `build-module-backend` untuk lanjutannya | 🟡 **`BE-BD-006` selesai sebagian 12 September 2026 — source lengkap, nol kriteria terbukti.** Sesudah build hijau: baseline EF, satu migration ber-scope `BbkBloodUnitAllocation`, inspeksi isinya, penerapan ke `QuilvianNewDevSukma` **hanya bila** tidak ada migration modul lain yang ikut terbawa, lalu sembilan skenario acceptance. **Riwayat:** 🟡 `READY` — siap dijadwalkan, dan dapat diselesaikan penuh secara mandiri. Acceptance final sembilan butir: `AC-BD-043/044/045/046/060/068/070` + `AC-BD-033` + konkurensi `VAL-BD-018c`. **Tidak** termasuk `AC-BD-071`, yang sudah dilepas ke `BE-BD-009` pada revisi 11 — artinya task ini **tidak boleh** membangun endpoint `reallocate`. Endpoint `allocate` wajib memanggil `EvaluateAllocationGateAsync` yang sudah ada (`02-backend-architecture.md` §F.4). Jalur kritis `BE-BD-006` → `BE-BD-007`. Bukti mengikuti kebijakan verifikasi bagian 0.1 roadmap: build produksi, inspeksi EF/skema, QBE, verifikasi manual API/DB terkendali. Satu task satu wewenang |
