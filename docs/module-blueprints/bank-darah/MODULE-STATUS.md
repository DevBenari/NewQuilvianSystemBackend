...
| --- | --- | --- |
| Backend | **16 dari 16 ✅**, 0 sebagian, 0 siap, 0 blocked, 0 future | `roadmap/backend-roadmap.md` bagian 3; 16 laporan di `task/report/backend/` |
...
| Frontend | 12 task = **0 ✅**, **3 🟡** (`FE-BD-001`, `FE-BD-006`, `FE-BD-011`), **9 belum dikerjakan**, **0 ⛔**. Seluruhnya berada di gelombang 1 | [frontend-roadmap.md](roadmap/frontend-roadmap.md) revisi 8 |
| `FE-BD-001` | **Diturunkan ✅ → 🟡.** Kedua hook editor memanggil `utils.unwrapApiData`, yang tidak ada pada objek default export utils-nya. Akibatnya simpan yang berhasil di server tampil "Gagal Menyimpan" di layar. **Task frontend pertama yang dibuka ulang** | Source frontend `6640a5e7`, `use-master-data-blood-components-editor.jsx` dan `use-master-data-blood-bank-reasons-editor.jsx` baris 153 dan 179 |
| `FE-BD-011` | 🟡 1 dari 2, **tidak lagi tertahan backend**. `HeldUnitCount` ada pada `GET /blood-storage-locations/{id}` | `BloodStorageLocationController.cs` baris 148–171 di `77f60c88` |
| `FE-BD-006` | 🟡 1 dari 2. Sumber hak akses backend kini ada (`GET /api/v1/Auth/permissions`), tetapi pembacanya di frontend belum ada (`BD-UI-GAP-002`) | `77f60c88`; nol pemakai di `6640a5e7` |
...
| Roadmap frontend | Revisi 8 **`APPROVED`** — `Sukmagp` 18 September 2026. `BD-UI-GAP-003` diserap ke acceptance `FE-BD-002`, `FE-BD-005`, `FE-BD-008`, dan `FE-BD-009`. `BD-UI-GAP-002` tetap gap implementasi, bukan penahan approval. **Riwayat:** `FORWARD-TEST / DRAFT`, butuh approval | Metadata `frontend-roadmap.md` |
...
| `BD-PH-008` | Implementasi frontend | **`IN_PROGRESS`** | **Per 18 September 2026 — fase berjalan:** 12 task = 0 ✅ + 3 🟡 (`FE-BD-001` diturunkan dari ✅, `FE-BD-006`, `FE-BD-011`) + 9 belum dikerjakan + 0 ⛔. Nol penahan backend. Roadmap revisi 8 **disetujui** `Sukmagp` 18 September 2026; task berikutnya: buka ulang `FE-BD-001`. **Riwayat (pagi 18 September 2026):** roadmap revisi 8 masih `DRAFT`, eksekusi menunggu approval. **Riwayat — per 10 September 2026:** tiga task frontend berlaporan — `FE-BD-001` ✅, `FE-BD-011` 🟡 1 dari 2 kriteria, `FE-BD-006` 🟡 1 dari 2 kriteria; `FE-BD-009` siap dijadwalkan. **Riwayat (`NOT_STARTED`):** Kontrak API **sudah** `approved` dan terkunci pada `v4`, sehingga gerbangnya tidak lagi menahan. Yang menahan tinggal urutan biasa: tidak ada task FE yang mendahului task BE pasangannya, dan belum ada satu pun task BE yang dijalankan. |
...
| --- | --- | --- |
| `BD-PH-001` sampai **`BD-PH-007`** | `BD-PH-008` berjalan — roadmap frontend revisi 8 disetujui 18 September 2026; task berikutnya buka ulang `FE-BD-001` · `BD-PH-009` berjalan | **Nihil** |
...
| --- | --- | --- | --- |
| **`DONE`** — 16 dari 16, 18 September 2026 | **`IN_PROGRESS`** — 0 ✅, 3 🟡, 9 belum | `NOT_STARTED` | **`IN_PROGRESS`** |
...
| `FE-BD-001` | 🟡 **SEBAGIAN — diturunkan 18 September 2026** | Simpan (tambah dan ubah) rusak pada kedua layar master (`utils.unwrapApiData` tidak ada pada default export). Buka ulang task yang sama. **Riwayat:** ✅ **`SELESAI`** 7 September 2026 |
| `FE-BD-011` | `SELESAI SEBAGIAN` 10 September 2026 | **18 September 2026:** tidak lagi tertahan backend; sisa pekerjaan murni frontend (`GET /{id}` → `HeldUnitCount`). **Riwayat:** 1 dari 2 kriteria; jumlah kantong tertahan **kini tersedia di backend** — `HeldUnitCount` pada detail lokasi sejak `BE-BD-015` — tetapi layar belum memakainya |
| `FE-BD-006` | `SELESAI SEBAGIAN` 10 September 2026 | 1 dari 2 kriteria; penyaringan menu menurut hak akses belum ada di frontend. **18 September 2026:** sumber backend `GET /api/v1/Auth/permissions` kini ada; yang kurang pembaca di frontend (`BD-UI-GAP-002`) |
...
| `FE-BD-002` | Belum dikerjakan — siap dijadwalkan (roadmap frontend revisi 8). **Riwayat:** 🟡 penahan backend-nya hilang 11 September 2026 | — (roadmap frontend revisi 8 disetujui 18 September 2026) |
...
### Current 18 September 2026 - sesudah approval roadmap frontend revisi 8
...
| 3 | **Buka ulang `FE-BD-001`** — perbaiki simpan pada kedua layar master | `build-module-frontend`, dengan wewenang tulis frontend khusus task ini | **Task frontend pertama yang dapat dijalankan sekarang.** Kecil, tanpa prasyarat, dan menahan pemakaian master yang dibutuhkan layar operasional. Laporan dan task ID yang sama |
| 4 | `FE-BD-011`, lalu `FE-BD-002` → `FE-BD-003` → `FE-BD-012` → `FE-BD-004` → `FE-BD-005` → `FE-BD-009` → `FE-BD-007` → `FE-BD-008` → `FE-BD-010` | `build-module-frontend`, satu task satu wewenang | Saran prioritas mengikuti perjalanan fisik kantong. Semuanya gelombang 1. Pembaca hak akses frontend (`BD-UI-GAP-002`) diputuskan pada task pertama yang benar-benar membutuhkannya |
