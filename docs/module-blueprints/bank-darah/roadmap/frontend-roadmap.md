...
| 10 | `FE-BD-008` 🟡 | Koreksi dua langkah dan tunggakan bukti darurat. **Sebagian per 24 September 2026**: source lengkap, runtime akun A 6 dari 6 `PASS`; skenario tahap kedua menunggu akun B (`E6`) ([laporan](../task/report/frontend/FE-BD-008.md)) |
...
| `FE-BD-008` | Koreksi dua langkah dan tunggakan bukti darurat | 3 | `BE-BD-010` ✅ [BE] | 🟡 **sebagian, 24 September 2026** — `lint:errors`/`build` `PASS` (371 halaman); `test:unit` 1619 test, 1612 lulus (8 baru lulus, 7 gagal lama); runtime **6 dari 6** skenario akun A `PASS` terhadap backend sungguhan (`R0`–`R3`, `R5`, `R9`). Belum terbukti: tahap kedua Setujui/Tolak oleh pemutus lain (`R4`, `R6`–`R8`), karena akun B untuk keputusan `E6` belum diberikan. Butir terpenuhi: worklist #3, `FE-BD-016`, catatan biaya; sebagian: dua langkah, `FE-BD-017` (sisi tersembunyi terbukti). DoD `NOT APPLICABLE`. **Riwayat:** belum dikerjakan | [laporan](../task/report/frontend/FE-BD-008.md) |

**Hitungan per 24 September 2026, sesudah `FE-BD-008` 🟡:** 12 task = **10 selesai** (`FE-BD-001`,
`FE-BD-011`, `FE-BD-006`, `FE-BD-002`, `FE-BD-003`, `FE-BD-010`, `FE-BD-004`, `FE-BD-012`, `FE-BD-009`, `FE-BD-007`) +
**2 sebagian** (`FE-BD-005`, **`FE-BD-008`**) + **0 belum dikerjakan** + **0 terblokir** pada register ini.
