...
| 7 | `FE-BD-005` ✅ | Golongan darah, bukti kecocokan, pemberian, dan jalur darurat. Juga membangun kerangka layar pemeriksaan `FE-BD-06`. ✅ **SELESAI — 25 September 2026 (final)** ([laporan](../task/report/frontend/FE-BD-005.md) bagian 6.5). Kesembilan butir outcome dan acceptance kartu terpenuhi: input golongan darah dari konteks pasien (detail kantong dan detail order → catat sampel → catat hasil 8 opsi → validasi), dan konflik golongan darah **sungguhan** membuat detail kantong menampilkan penanda serta menahan Berikan dan Jalur Darurat lewat `BE-BD-022` ✅. `lint:errors` `PASS` (warning per berkas sama dengan `HEAD`), `build` `PASS` (371 halaman), `test:unit` 1630 test — 1623 lulus (6 baru lulus, 7 gagal lama); **runtime `E0`–`E8` 8 dari 8 `PASS`** terhadap backend sungguhan, konflik dipulihkan. DoD kartu `NOT APPLICABLE` (kartu tanpa baris DoD); DoD pemilik terpenuhi. Belum di-commit. **Riwayat:** 🟡 **Diperbarui 25 September 2026 — lanjutan sesudah `BE-BD-021` ✅** ([laporan](../task/report/frontend/FE-BD-005.md) bagian 6.4): gerbang pemberian terbaca dan Berikan tertahan **sebelum** ditekan, masa berlaku bukti tampil, gerbang jalur darurat ditetapkan dari proyeksi, penanda konflik golongan darah pasien tujuan. `lint:errors` `PASS`, `build` `PASS` (371 halaman), `test:unit` 1624 test — 1617 lulus (5 baru lulus, 7 gagal lama), **runtime `G0`–`G9` 10 dari 10 `PASS`** terhadap backend sungguhan. **7 dari 9 butir terpenuhi** (`FE-BD-008`, `FE-BD-013`, `FE-BD-021` kini terpenuhi). **Tetap 🟡:** outcome mencatat golongan darah (di luar cakupan keputusan pemilik), dan `FE-BD-007` sebagian — penanda terlihat (bukti positif tersimulasi), tetapi tidak menahan karena backend tidak menegakkan `VAL-BD-034` pada kantong. Belum di-commit. **Riwayat:** **Sebagian per 24 September 2026**: bukti, pemberian, dan jalur darurat pada detail kantong; runtime `R0`–`R9` **14 dari 14 `PASS`** di Chromium terhadap backend sungguhan; layar `FE-BD-06` tidak dibangun atas keputusan pemilik ([laporan](../task/report/frontend/FE-BD-005.md)) |
...
| 10 | `FE-BD-008` ✅ | Koreksi dua langkah dan tunggakan bukti darurat. **Selesai 24 September 2026**: runtime 10 dari 10 `PASS` dengan dua akun (`E6`) ([laporan](../task/report/frontend/FE-BD-008.md)). **Riwayat:** sebagian — tahap kedua menunggu akun B |
...
| `FE-BD-008` | Koreksi dua langkah dan tunggakan bukti darurat | 3 | `BE-BD-010` ✅ [BE] | ✅ **selesai 24 September 2026** — `lint:errors`/`build` `PASS` (371 halaman); `test:unit` 1619 test, 1612 lulus (8 baru lulus, 7 gagal lama); runtime **10 dari 10** skenario `PASS` terhadap backend sungguhan dengan dua akun (`E6`: A `superadmin` mengajukan, B `arif@admin.com` menolak/menyetujui). Kedua langkah, worklist #3, `FE-BD-016`, `FE-BD-017`, dan catatan biaya terbukti. DoD `NOT APPLICABLE`. **Riwayat:** 🟡 sebagian — runtime akun A 6 dari 6, tahap kedua menunggu akun B; belum dikerjakan | [laporan](../task/report/frontend/FE-BD-008.md) |

**Hitungan per 25 September 2026, sesudah `FE-BD-005` ✅ final:** 12 task = **12 selesai** — seluruh task frontend Bank Darah pada register ini selesai; **0 sebagian** + **0 belum dikerjakan** + **0 terblokir**.

**Riwayat — hitungan per 24 September 2026, sesudah `FE-BD-008` ✅:** 12 task = **11 selesai** (`FE-BD-001`,
`FE-BD-011`, `FE-BD-006`, `FE-BD-002`, `FE-BD-003`, `FE-BD-010`, `FE-BD-004`, `FE-BD-012`, `FE-BD-009`, `FE-BD-007`,
**`FE-BD-008`**) + **1 sebagian** (`FE-BD-005`) + **0 belum dikerjakan** + **0 terblokir** pada register ini.

**Riwayat — hitungan per 24 September 2026, sesudah `FE-BD-008` 🟡:** 12 task = **10 selesai** + **2 sebagian**
(`FE-BD-005`, `FE-BD-008`) + **0 belum dikerjakan** + **0 terblokir** pada register ini.

**Riwayat — hitungan per 24 September 2026, sesudah `FE-BD-007` ✅:** 12 task = **10 selesai** (`FE-BD-001`,
`FE-BD-011`, `FE-BD-006`, `FE-BD-002`, `FE-BD-003`, `FE-BD-010`, `FE-BD-004`, `FE-BD-012`, `FE-BD-009`, **`FE-BD-007`**) +
...
| Field | Isi |
| --- | --- |
| **Status** | ✅ **SELESAI 24 September 2026** ([laporan](../task/report/frontend/FE-BD-008.md)). **Runtime 10 dari 10 skenario `PASS`** (run 4) dengan dua akun sesuai `E6`: akun A `superadmin` mengajukan, akun B `arif@admin.com` (bukan SuperAdmin, memegang `ApproveCorrection`, tanpa `Correct`) menolak dan menyetujui. `FE-BD-017` terbukti dari kedua sisi: tersembunyi pada koreksi sendiri (`R2`, `R5`), tampil bagi pemutus lain (`R4`). `FE-BD-016`: pemenuhan backend 1 → 1 (diajukan) → 1 (ditolak) → 0 (disetujui). **Riwayat:** 🟡 Sebagian — 24 September 2026, tahap kedua menunggu akun B. Keputusan pemilik `Sukmagp` 24 September 2026: `E1` preset `emergencyPendingEvidence=true`, dengan backlog endpoint penyusulan bukti sesudah pemberian darurat; `E2` preset sebagai sumber penanda, penanda tabel dan hitungan ringkasan jadi backlog; `E3` status, waktu pengajuan/keputusan, dan label **Anda**, tanpa GUID; `E6` runtime dua akun. `npm run lint:errors` `PASS`; `npm run build` `PASS`, 371 halaman; `npm run test:unit` 1619 test — 1612 lulus termasuk 8 test baru, 7 kegagalan lama sama dengan baseline. **Runtime 6 dari 6 skenario akun A `PASS`** di Chromium terhadap backend sungguhan (`QuilvianNewDevSukma`): pengajuan dengan isi tepat kontrak, status **Menunggu persetujuan** + **Anda**, pemenuhan backend tetap (`FE-BD-016`), tombol keputusan tersembunyi pada koreksi sendiri walaupun pengguna sungguh memegang `ApproveCorrection` (`FE-BD-017`, diperkuat `422 VAL-BD-073` lewat API), dan preset tunggakan (5 kantong, sama dengan backend). **Belum terbukti:** Setujui/Tolak oleh pemutus lain (`R4`, `R6`–`R8`) — akun B belum diberikan. Baris DoD `NOT APPLICABLE` (kartu tanpa DoD). **Riwayat:** Belum dikerjakan — siap dijadwalkan (revisi 8 disetujui 18 September 2026). Nol source di frontend `6640a5e7`. `BE-BD-010` ✅ ([laporan](../task/report/backend/BE-BD-010.md)). **Riwayat:** 🟡 PENDING (refresh dependency 17 September 2026); ⛔ BLOCKED — menunggu `BE-BD-010`, yang **READY tetapi belum diimplementasikan** (per 17 September 2026; `OQ-BD-014` ditutup `DEC-BD-051`); `BE-BD-010` tertahan lewat `BE-BD-007` |
...
| **3 — Kantong darah, pemberian, dan penyelesaiannya** | Kantong disimpan, dialokasikan, diberikan, diselesaikan, dan dikoreksi dari layar | `FE-BD-012`, `FE-BD-004`, `FE-BD-005`, `FE-BD-009`, `FE-BD-007`, `FE-BD-008` | ✅ **Selesai 25 September 2026** — 6 dari 6 selesai; terakhir `FE-BD-005` ✅ final ([laporan](../task/report/frontend/FE-BD-005.md) bagian 6.5). **Riwayat:** 🟡 **Berjalan per 24 September 2026** — 5 dari 6 selesai (termasuk `FE-BD-008` ✅, lihat di bawah): `FE-BD-004` ✅ ([laporan](../task/report/frontend/FE-BD-004.md)), `FE-BD-012` ✅ ([laporan](../task/report/frontend/FE-BD-012.md)), `FE-BD-009` ✅ ([laporan](../task/report/frontend/FE-BD-009.md), runtime 11 dari 11 `PASS`), dan `FE-BD-007` ✅ ([laporan](../task/report/frontend/FE-BD-007.md), runtime 13 dari 13 `PASS`). Sebagian: `FE-BD-005` 🟡 ([laporan](../task/report/frontend/FE-BD-005.md), runtime 14 dari 14 `PASS`; 4 butir kartu tertahan data backend) **Selesai:** `FE-BD-008` ✅ ([laporan](../task/report/frontend/FE-BD-008.md), runtime 10 dari 10 `PASS` dengan dua akun). **Riwayat:** `FE-BD-008` 🟡, tahap kedua menunggu akun B. **Riwayat:** belum dikerjakan: `FE-BD-008`. **Riwayat:** 3 dari 6 selesai; belum dikerjakan `FE-BD-007`, `FE-BD-008`. **Riwayat:** 2 dari 6 selesai; belum dikerjakan `FE-BD-009`, `FE-BD-007`, `FE-BD-008`. **Riwayat:** Belum dikerjakan: `FE-BD-005`, `FE-BD-009`, `FE-BD-007`, `FE-BD-008`; 1 dari 6 — `FE-BD-012` tertahan menunggu `BE-BD-020`; Belum dikerjakan |
