...
| Field | Isi |
|---|---|
| **Status** | 🟡 **`PARTIAL`** — 11 September 2026. **Fase A** (pemecahan 22 identitas di source, commit `85fcc3fd` 4 September) dan **Fase A′** (koreksi `DoctorConsultation.WriteSoap` menjadi identitas ke-23, perbaikan paritas `IsCancel`, regression test, skrip diagnostik baca-saja) **selesai**. **Fase B** (perluasan `SysAccessPolicy`) dan **Fase C** (penyempitan audio antrean) **belum dijalankan**. **2 dari 10 acceptance criteria terpenuhi** (nomor 1 dan 3); nomor 2, 4, 5, 6, 9 menunggu Fase B/C; nomor 7, 8, 10 tidak dapat dibuktikan karena gerbang test mati. **Blocker:** proyek test `UnitTests.InMemory` gagal dikompilasi — 14 galat pada dua berkas milik modul lain (`BillingDepositServiceTests.cs` 11 galat, `PatientEncounterCompanyGuarantorTests.cs` 3 galat) — sehingga seluruh test keamanan ikut mati. `dotnet build` proyek utama **berhasil**, dan keempat berkas yang diubah task ini kompilasi bersih; keadaan database belum terukur. Laporan: [`../task/report/backend/BE-SEC-003.md`](../task/report/backend/BE-SEC-003.md) <br><br> **PEMBARUAN 17 September 2026 (`BE-SEC-018`).** **Fase B SELESAI pada database development** `QuilvianNewDevAndryZain`: Tahap 1 menghasilkan 35 pelestarian historis + 1 `PatientAssessment.Amend` = **36**, Tahap 2 menonaktifkan tepat **4** policy warisan (`PatientProcedure.Update` 1, `DoctorQueue.Update` 3). Keduanya disimulasikan `ROLLBACK` lebih dulu, lalu di-commit. Registry database kini sejajar dengan source pada **1.300 / 340 / 48**. **Fase C (penyempitan audio antrean) TETAP BELUM dikerjakan** — identitas `QueueVoice.PlayAudio` belum ada di source. Status karena itu **tetap `PARTIAL`**. Blocker gerbang test sudah tidak berlaku apa adanya: backend test project dihapus merge Integration dan sebagian invariannya kini dijaga authorization verifier (`BE-SEC-012`), tetapi pemetaan lengkap acceptance criteria 7/8/10 ke invarian verifier **belum ditinjau ulang**. Acceptance criterion nomor **6 (rollback teruji) masih terbuka** — skrip balik belum pernah dijalankan terhadap database. Bukti eksekusi: [`../task/report/backend/BE-SEC-018.md`](../task/report/backend/BE-SEC-018.md) |
...
| **Gerbang terbuka** | ~~Kontrak `1 + 3 = 4` belum pernah dikonfirmasi~~ → ✅ dikonfirmasi terhadap database `BE-SEC-018` 17 September 2026. `CompanyGuarantor.*` **masih** belum dianalisis siapa pun |

---

### `BE-SEC-018` ✅ — Closeout eksekusi pemeliharaan otorisasi pada database development

| Field | Nilai |
|---|---|
| **Status** | ✅ **Selesai** 17 September 2026 — **seluruh urutan tulis pemeliharaan otorisasi SUDAH DIJALANKAN dan DI-COMMIT** terhadap `QuilvianNewDevAndryZain`. Task ini **dokumentasi/bukti saja**: nol perubahan source aplikasi, nol perubahan semantik SQL, nol akses database. Build `Release` `0 Error(s)`; authorization verifier `PASS`. Laporan: [`BE-SEC-018.md`](../task/report/backend/BE-SEC-018.md) |
| **Database** | `QuilvianNewDevAndryZain` · SHA sumber saat pelaksanaan `cb7f3fce` · backup `dump-QuilvianNewDevAndryZain-202609171439.dump` dibuat **sebelum** operasi tulis |
| **1 — Pencabutan Finance** | `Finance` × `Manajer Finance` × `WorkSchedule` `Update`/`Delete` dinonaktifkan. Efektif **0**, baris nonaktif dipertahankan **2**, nol baris dihapus, `UpdateBy` = superadmin. `Read`/`Create` Finance **tidak disentuh** |
| **2 — `AccessMenuSeeder`** | Dijalankan lewat **runner sementara eksternal**; aplikasi lengkap **tidak** dinyalakan, sehingga tidak ada seeder lain, hosted service, migration, maupun traffic HTTP. Modul 48 → **48**, resource 339 → **340** (+1), action 1.286 → **1.300** (+14). `RoutePath` NULL tetap **1** sebelum dan sesudah — membuktikan runner memakai jalur host MVC, bukan `BuildFromAssembly` |
| **3 — Pemberian hak HR** | Dry-run: 36 sasaran, 32 akan disisipkan, 4 sudah ada. Akhir: `Manajer HR` **24**, `Staff HR` **12**, `Staff HR` `Update`/`Delete` **0**, duplikat kunci alami **0** |
| **4 — `BE-SEC-003B` Tahap 1** | Disimulasikan `ROLLBACK` lebih dulu, lalu di-commit. Akhir: pelestarian historis **35** + `PatientAssessment.Amend` **1** = **36**. `Complete` efektif **1** |
| **5 — `BE-SEC-003B` Tahap 2** | Disimulasikan `ROLLBACK` lebih dulu, lalu di-commit. `PatientProcedure.Update` aktif **0**/nonaktif **1**; `DoctorQueue.Update` aktif **0**/nonaktif **3**. Kontrak `1 + 3 = 4` terbukti terhadap database |
| **Acceptance akhir** | `1300 / 340 / 48`; legacy aktif **0**, dinonaktifkan **4**; manajer HR **24**; staff HR **12**; staff terlarang **0**; Finance write efektif **0**; `Complete` **1**; `Amend` **1**; duplikat **0**. **Seluruh nilai cocok** |
| **Di luar scope** | `BillingItemCategory.*`, `CompanyGuarantor.*`, `KioskScanSession.Cancel`, `Queue.*` — **tidak satu pun** dihapus, diaktifkan, dinonaktifkan, atau dimigrasikan |
| **⛔ Peringatan** | **Urutan tulis pemeliharaan SELESAI. Jangan dijalankan ulang secara buta.** Gerbang kardinalitas pada kedua skrip pencabutan sengaja membatalkan diri pada eksekusi kedua — itu perilaku yang diinginkan, jangan dilonggarkan. Lingkungan selain development **belum disentuh** dan menuntut urutan, backup, serta persetujuan tersendiri |
| **Berikutnya** | `BE-SEC-003` **Fase C** — penyempitan audio antrean. Lihat [`BE-SEC-018.md`](../task/report/backend/BE-SEC-018.md) bagian 13 |
