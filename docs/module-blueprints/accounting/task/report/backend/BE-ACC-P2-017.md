...
| Status | **✅ SELESAI 15 September 2026** — 6 dari 6 acceptance terpetakan ke source; build owner 0 error; butir Verifikasi terakhir terpenuhi: `GET /event-types` dan `GET /posting-rules` dipanggil owner dengan sesi login dan menjawab **`200`**, bentuk responsnya cocok penuh dengan DTO (bagian 5). UAT belum dijalankan. **Riwayat:** 🟡 SEBAGIAN 14 September 2026 — 6 dari 6 acceptance terpetakan ke source. Dari dua butir verifikasi yang semula belum, **build ulang sesudah perbaikan log sudah terpenuhi** (0 error, 192 warning) dan migration `BE-ACC-P2-016` sudah diterapkan. **Sisa satu-satunya:** uji panggil `GET /event-types` dan `GET /posting-rules` saat backend berjalan — belum dapat dijalankan karena port 5107 tertutup saat diperiksa 14 September 2026 13.17 WIB. **Pembaruan 15.20 WIB:** backend berjalan dan kedua rute terbukti terdaftar (`401` tanpa login, rute kontrol `404`); respons `200` dengan login belum ada, jadi tetap 🟡 |
...
| **Uji kontrak read-only `GET /api/v1/corporate/accounting/event-types` — owner, Swagger, 15 Sep 2026 08.30 WIB** | **`200`** — `success: true`, `message` "Daftar jenis kejadian berhasil diambil.", `data` berhalaman `pageNumber 1`, `pageSize 25`, `totalData 1`, `totalPage 1`. Satu butir memuat tepat 7 bidang `EventTypeListResponse` (`EventTypeDtos.cs` baris 31–50): `id`, `eventTypeCode` `PATIENT_PAYMENT`, `eventTypeName`, `sourceModule` `CASHIER`, `isActive` `true`, `activePostingRuleCount` `0`, `createDateTime`. Nol bidang lebih, nol bidang kurang | `PASS` | Tangkapan layar owner |
| **Uji kontrak read-only `GET /api/v1/corporate/accounting/posting-rules` — owner, Swagger, 15 Sep 2026 08.41 WIB** | **`200`** — `message` "Daftar aturan posting berhasil diambil.", `totalData 1`. Satu butir memuat tepat 13 bidang `PostingRuleListResponse` (`PostingRuleDtos.cs` baris 39–65): `id`, `legalEntityId`, `legalEntityName` "PT Metropolitan Medical Centre", `eventTypeId`, `eventTypeCode`, `eventTypeName`, `journalTypeId`, `journalTypeCode` `JU`, `journalTypeName`, `treatment` **`2`** (angka, `BuatDraft`), `isActive`, `lineCount` `2`, `createDateTime`. Durasi 174 ms | `PASS` | Tangkapan layar owner |
| Yang dibuktikan kedua `200` itu | (1) tabel `AccEventType`, `AccPostingRule`, `AccPostingRuleLine` ada pada database yang dipakai backend — tidak lagi `42P01`; (2) `[AccessPermission]` meloloskan sesi itu — lewat hak yang dicentang atau lewat jalur SuperAdmin; tangkapan layar tidak membedakan keduanya, sehingga pemberian hak bagi pengguna non-SuperAdmin **tidak** dianggap terbukti di sini; (3) bentuk respons sama dengan DTO yang dikonsumsi `FE-ACC-P2-009`/`010`; (4) `eventTypeId` pada aturan posting sama dengan `id` jenis kejadian — relasi terbaca | `PASS` | — |
| Catatan data | Satu jenis kejadian `PATIENT_PAYMENT` dan satu aturan posting berbaris dua sudah dibuat owner lewat aplikasi, bukan oleh agent. Isi datanya tetap keputusan `DEC-ACC-P2-002`; task ini tidak menilai kebenaran pemetaan akunnya | Dicatat | — |
| **Uji kontrak read-only — riwayat 14 Sep 2026** | Belum dijalankan — backend tidak berjalan. `016` ✅ sudah diterapkan, jadi penghalangnya kini hanya proses backend, bukan skema. Rencana ujinya: login, lalu `GET` saja — tanpa `POST`, `PUT`, atau `PATCH` | `NOT RUN` | Menunggu backend dijalankan |
...
| Pemanggilan endpoint sesudah `016` diterapkan | **Ya** — 15 Sep 2026, kedua `GET` menjawab `200` dengan bentuk cocok DTO. Riwayat: 14 Sep 13.17 WIB backend tidak berjalan; 15.20 WIB rute terbukti terdaftar (`401`/`404`); pagi 14 Sep `016` belum dibuat |
| Source berubah | **Ya** |
...
3. ~~**Rizki:** jalankan backend dengan pengguna yang berhak `EventType : Read` dan `PostingRule : Read`.~~ **Selesai 15 September 2026.**
4. ~~Uji kontrak read-only.~~ **Selesai 15 September 2026** — dijalankan owner lewat Swagger, keduanya `200`; task naik ke ✅. Sisa di luar task ini: beri hak `EventType : *` pada database yang departemennya terisi untuk pengguna non-SuperAdmin, dan UAT oleh tim UAT. Teks rencana semula dipertahankan: agent lalu menjalankan uji kontrak **read-only**: login, `GET /api/v1/corporate/accounting/event-types` dan `GET /api/v1/corporate/accounting/posting-rules` — tanpa `POST`, `PUT`, atau `PATCH`. Bila keduanya menjawab `200` dengan bentuk `PagedResult` yang cocok dengan bagian 4 laporan ini dan bagian 4 [`BE-ACC-P2-018`](BE-ACC-P2-018.md), laporan ini diperbarui dan task naik ke ✅. Riwayat: rencana semula menyebut `POST /event-types`; dipersempit menjadi `GET` saja atas instruksi owner 14 September 2026.
