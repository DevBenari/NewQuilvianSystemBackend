...
| ~~`ACC-GAP-011`~~ | ~~`JournalApprovalResponse` tidak memuat nama penyetuju~~ — **`CLOSED` 4 September 2026** lewat `BE-ACC-015`, pilihan A yang diputuskan owner. `ActionByName` ditambahkan pada riwayat, ditambah `SubmittedByName`, `ApprovedByName`, dan `PostedByName` pada rincian — cakupannya ternyata **4 field, bukan 1**. Nama diambil saat baca dalam satu kueri batch, tidak dipersistensi (`QBE-ENT-003`). `ACC-API` naik `0.4` → `0.5`. Nol migration, nol perubahan skema | — | Rizki |
