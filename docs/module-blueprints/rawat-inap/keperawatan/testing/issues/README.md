...
| **`ISSUE-KEP-007`** | [`ISSUE-KEP-007-kegagalan-simpan-evaluasi-awal-mpp-resolusi-version-id-checklist.md`](./ISSUE-KEP-007-kegagalan-simpan-evaluasi-awal-mpp-resolusi-version-id-checklist.md) | Kegagalan simpan konsep Evaluasi Awal MPP (`HTTP 409 Conflict` - `INSTRUMENT_VERSION_CHANGED`) akibat hook membaca properti `instrumentVersionId` bukannya `versionId` dari respon resolusi checklist. | 🔴 Tinggi / API Contract | 🟢 **RESOLVED** (24 Sep 2026) |
| **`ISSUE-KEP-008`** | [`ISSUE-KEP-008-deserialisasi-status-kesadaran-dan-wewenang-tanda-vital.md`](./ISSUE-KEP-008-deserialisasi-status-kesadaran-dan-wewenang-tanda-vital.md) | Kegagalan penyimpanan Tanda Vital (`HTTP 400 Bad Request`) akibat nilai enum non-nullable `ConsciousnessStatus` menerima `null`, ketiadaan pemilih kesadaran di form, serta ketiadaan wewenang akses perawat pada kontroler `PatientVitalSign`. | 🔴 Tinggi / API & RBAC | 🟢 **RESOLVED** (24 Sep 2026) |
