...
| Status | ✅ Selesai — enam kriteria terpetakan; `dotnet build` lolos; migration K1 **diterapkan** ke `QuilvianNewDevHamzah` dan skema terverifikasi dari katalog; keluaran seeder `NOT RUN` |
...
| Database | Migration `20260917100000_AddClinicalInstrument` (K1) — ditulis tangan beserta snapshot; **diterapkan ke `QuilvianNewDevHamzah` 17 September 2026** |
...
| `dotnet ef migrations has-pending-model-changes`, verifikasi skema | Model = snapshot; skema dibaca dari katalog `QuilvianNewDevHamzah` (bukan Postgres sekali pakai) | `PASS` | Lampiran |
...
| DoD: verifikasi skema Postgres sekali pakai dan `dotnet build` | `dotnet build` lolos; skema terverifikasi dari katalog | `PASS` |
...
Migration task ini: `20260917100000_AddClinicalInstrument` (K1) — diterapkan ke `QuilvianNewDevHamzah` 17 September 2026.
