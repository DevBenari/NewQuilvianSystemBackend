...
| Source SHA pass ini | Backend `34f368d4` (branch `MHamzah`); frontend `147355f5` (branch `HamzahV2`). Capability map **basi** terhadap keduanya; impact scan `RLN-PH-03` ditawarkan dan boleh berjalan bersamaan |
| Tanggal pass | 2026-09-14 — sebelumnya 2026-09-02 |
...
| Keputusan tercatat | 90 | `RWI-DEC-093` s.d. `RWI-DEC-096` disetujui pada Amendment Pass deposit admisi 2026-09-08; `RWI-DEC-075` diamandemen, bukan digantikan. Sebelumnya `RWI-DEC-090`, `RWI-DEC-091`, dan `RWI-DEC-092` disetujui pada Amendment Pass approval Keperawatan; `RWI-DEC-004` dan `RWI-DEC-034` menjadi `superseded`. Sebelumnya `RWI-DEC-089` pada Amendment Pass pemakaian alat. Sebelumnya `RWI-DEC-086` s.d. `RWI-DEC-088` pada Amendment Pass koreksi dokumen. `RWI-DEC-018`, `RWI-DEC-025`, dan `RWI-DEC-031` kini `superseded` penuh |
| Fakta yang terbukti dari repository dan PRD | 19 | `RWI-FACT-001` sampai `RWI-FACT-019`. Tiga terakhir dibaca dari source `44099e4` saat trace ulang deposit 2026-09-08. Dua terakhir dibaca langsung dari source pada Amendment Pass pemakaian alat dan Amendment Pass approval Keperawatan |
| Acceptance criteria tercatat | 180 | `RWI-AC-001` sampai `RWI-AC-180`. Empat butir historis `RWI-AC-032`, `033`, `047`, dan `048` tidak current setelah `RWI-DEC-084`–`085` |
...
| Pertanyaan wawancara tersisa | **13 untuk dokter dan keperawatan, per 2026-09-14** — dua di antaranya dijawab sebagian | **Diperbarui 2026-09-14:** `PRD-RWI-V2-001` membuka `RWI-OQ-056` s.d. `RWI-OQ-068`; `RWI-OQ-064` dan `RWI-OQ-065` dijawab sebagian oleh `RWI-DEC-108`. Keputusan baru `RWI-DEC-106` s.d. `RWI-DEC-109`, fakta baru `RWI-FACT-023` dan `RWI-FACT-024`. Sebelumnya: `RWI-OQ-050` ditutup `RWI-DEC-088`; `RWI-OQ-048` ditutup `RWI-DEC-089`. Pertanyaan terbuka modul lain tidak dibahas pada pass ini |
...
### Batas scope Amendment Pass `PRD-RWI-V2-001` — MENUNGGU KONFIRMASI 2026-09-14
...
| `RWI-OQ-056` | **Alias `OD-RWI-001`.** Instrumen risiko jatuh final apa yang dipakai, termasuk versi, cara menghitung skor, dan batas kategori Rendah/Sedang/Tinggi | `DESIGN` untuk menu Resiko Jatuh keperawatan | `TERBUKA` — keputusan klinis; PRD bagian 29 mensyaratkan pengesahan pemilik klinis |
| `RWI-OQ-057` | **Alias `OD-RWI-002`.** Isian mana yang wajib terisi sebelum setiap jenis pengkajian boleh diselesaikan | `IMPLEMENTATION` — validasi penyelesaian `BR-RWI-005` | `TERBUKA` — keputusan klinis |
| `RWI-OQ-058` | **Alias `OD-RWI-003`.** Apakah verifikasi CPPT boleh didelegasikan, kepada siapa, dan dengan batas apa. Beririsan dengan `RWI-DEC-088` (koreksi atas nama dokter berhalangan) dan `RWI-RULE-021` | `DESIGN` untuk tab CPPT | `TERBUKA` — keputusan klinis dan Product |
| `RWI-OQ-059` | **Alias `OD-RWI-004`.** Bentuk dan isi Resume ODC (*One Day Care*), dan bedanya dengan resume rawat inap | `DESIGN` untuk tab Resume Medis | `TERBUKA` |
| `RWI-OQ-060` | **Alias `OD-RWI-005`.** Siapa pemilik rekonsiliasi obat. Beririsan dengan MAR yang dijadikan `P0/P1` oleh `RWI-DEC-097` | `DESIGN` untuk Obat & Alkes keperawatan | `TERBUKA` — Product bersama pemilik `PharmacyManagement` |
| `RWI-OQ-061` | **Alias `OD-RWI-006`.** Siapa pemilik template resep: pribadi per dokter, per kelompok spesialis, atau seluruh rumah sakit | `DESIGN` untuk tab Resep | `TERBUKA` |
| `RWI-OQ-062` | **Alias `OD-RWI-007`.** Apa definisi Resep Harian dan bedanya dengan resep biasa | `DESIGN` untuk tab Resep | `TERBUKA` |
| `RWI-OQ-063` | **Alias `OD-RWI-008`.** Siapa pemilik Evaluasi Awal / Manajemen Pelayanan Pasien. PRD bagian 33 melarang langsung memetakannya menjadi pengkajian awal keperawatan | `DESIGN` untuk menu Evaluasi Awal | `TERBUKA` |
...
| `RWI-OQ-065` | **Alias `OD-RWI-010`.** Siapa boleh melihat ringkasan tagihan dan apa isinya | `LATER SLICE` | **`DIJAWAB SEBAGIAN`** oleh `RWI-DEC-108`: menu tampil sebagai "Integrasi belum tersedia" sampai kontrak Billing ada. Isi dan hak lihatnya belum diputuskan; PRD bagian 47 mengusulkan ringkasan baca-saja sebagai bawaan |
| `RWI-OQ-066` | **Temuan `RLN-01`.** Bagaimana `PRD-RWI-V2-001` v`2.0` dibaca bersama `PRD-RWI-FINAL-001` v`1.0.0` dan `PRD-to-MVP-Rawat-Inap-V2` v`1.0.0`: menggantikan penuh, atau berlapis dengan PRD baru menang saat bertentangan | `DESIGN` untuk seluruh `RLN-PH-06` | `TERBUKA` — Pertanyaan 1 Amendment Pass `PRD-RWI-V2-001` |
| `RWI-OQ-067` | Pasien siapa saja yang tampil pada Daftar Pasien Rawat Inap di ruang kerja dokter `RWI-DEC-107` | `DESIGN` + privasi | `TERBUKA` |
| `RWI-OQ-068` | Untuk menu yang backend-nya **sudah ada** di sub-modul atau modul lain — Transfer Pasien dan Penunjang Laboratorium/Radiologi di keperawatan — apakah menu itu memakai kemampuan yang sudah ada atau ikut tampil "belum tersedia" | `DESIGN` | `TERBUKA` |
