...
| `dotnet build` | Tidak dijalankan | `NOT RUN` | Instruksi eksplisit pemilik: build dijalankan mandiri |
...
| Risiko tersisa | Belum ada bukti runtime sampai build dan uji dijalankan pemilik |
| Perubahan sampingan | `NONE` |
| Interupsi | `NONE` |
| Status Git | Lihat laporan gabungan pada jawaban sesi; berkas task ini berstatus `M` |
| Langkah berikutnya | Jalankan `dotnet build`; uji `409` pada konsep `LockedUnsigned` setelah penutupan episode |
