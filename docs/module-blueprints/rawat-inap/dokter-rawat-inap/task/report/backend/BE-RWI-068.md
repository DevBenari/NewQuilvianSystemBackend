...
| Status | ✅ **SELESAI 12 September 2026.** ~~🟡 SEBAGIAN 9 September 2026~~. Kesembilan acceptance criteria terpetakan ke source yang benar-benar ada dan terkompilasi; `dotnet build` solution **0 error** `CS` dengan 187 warning lama pada berkas yang tidak disentuh. **Status dinaikkan atas instruksi pemilik 12 September 2026:** pekerjaan implementasi backend dinyatakan cukup ditandai selesai begitu coding-nya ada, tanpa menunggu build atau uji. Tiga butir verifikasi tetap **`NOT RUN` dan ditulis apa adanya, tidak dihilangkan**: `dotnet test`, uji migration maju-mundur terhadap PostgreSQL, dan kompilasi berkas uji baru. **Satu akibat operasional yang wajib dibawa:** migration `20260909065125_AddInpatientEpisodeContextToPatientDiagnosis` **belum diterapkan ke database mana pun**, dan langkah mundurnya tidak simetris — mengembalikan `ConsultationId` menjadi `NOT NULL` akan gagal begitu ada satu baris diagnosis rawat inap tanpa nomor konsultasi. Penerapannya tetap menunggu wewenang terpisah. Verifikasi source 12 September 2026: `InpEpisodeId` ada pada `TrxPatientDiagnosis.cs:56` dan pada keempat DTO, `ConsultationId` sudah `Guid?` pada keempat titiknya |
