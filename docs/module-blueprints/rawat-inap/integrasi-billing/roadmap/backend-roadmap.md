...
- **Definition of Done (DoD):** `dotnet ef migrations add` sukses, script SQL diverifikasi, `dotnet build` 0 error.
...
- **Definition of Done (DoD):** Interface dan implementasi terdaftar di DI `Program.cs`, unit test transaksi lulus.
...
- **Definition of Done (DoD):** Hosted service terdaftar di `Program.cs`, integration test retry lulus.
...
- **Definition of Done (DoD):** Logika validasi terpasang di `InpatientBedTransferService`, unit test penolakan CLOSED lulus.
...
- **Kriteria Penerimaan (AC):**
  ...
  3. Endpoint `GET /episodes/{id}/billing-details` mengembalikan breakdown angka rupiah dan memvalidasi izin `InpatientBilling:View`. Pengguna tanpa izin mendapat respons `403 Forbidden`.
- **Definition of Done (DoD):** Controller terdaftar di Swagger dengan tag `[Tags("Inpatient Billing Operational")]`, authorization test lulus.
...
- **Definition of Done (DoD):** Endpoint webhook beroperasi, integration test auto-reblock lulus.
...
- **Kriteria Penerimaan (AC):**
  ...
  2. Override yang sah mencatat audit log permanen dan membuka izin pelepasan fisik darurat.
  ...
  4. Pelepasan fisik ditolak dengan kode 422 bila clearance masih `Pending`/`Revoked` tanpa override supervisor (`VAL-INT-001`).
- **Definition of Done (DoD):** Controller terdaftar di Swagger dengan tag `[Tags("Inpatient Discharge Clearance")]`, acceptance test `RWI-AC-238` & `239` lulus 100%.
