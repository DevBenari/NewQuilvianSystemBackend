...
| `PHA-DEC-058` | Decision | Copy resep ditunda. Ia dokumen legal, bukan sekadar fitur cetak, sehingga tidak boleh dibuat tanpa data pendukungnya. Belum boleh dibuat: template copy resep final, nomor copy resep, tanda `det`/`nedet`, dan validasi legal dokumen | Product/domain owner | `approved`; status `BLOCKED_BY_BUSINESS_DECISION` | Keputusan 3 September 2026 |
| `PHA-DEC-059` | Decision | Copy resep memerlukan kapabilitas baru **Prescription Item Dispensing History** dengan bentuk Prescription → Prescription Item → Dispensing Record → Remaining Quantity. Copy resep wajib mengambil datanya dari histori itu, bukan dari status resep. Dicatat sebagai backlog, belum diimplementasikan | Product/domain owner | `approved`; belum dijadwalkan | Keputusan 3 September 2026 |
| `PHA-OQ-021` | Open Question | Sumber Nomor SIA (Surat Izin Apotek) untuk instalasi farmasi. Tidak ada kolomnya di master mana pun saat ini | Pharmacy + legal/perizinan owner | `open`; `BLOCKING` untuk copy resep | `PHA-DEC-058`, 3 September 2026 |
| `PHA-OQ-022` | Open Question | Sumber data apoteker penanggung jawab: nama dan Nomor SIPA. Tidak ada kolomnya di master mana pun saat ini | Pharmacy + HR owner | `open`; `BLOCKING` untuk copy resep | `PHA-DEC-058`, 3 September 2026 |
| `PHA-OQ-023` | Open Question | Bentuk status penyerahan per item resep: sudah diserahkan, belum diserahkan, jumlah yang sudah diserahkan, dan jumlah sisa. `TrxPrescriptionItem` hanya memiliki `CoverageStatus` yang menyatakan penjaminan, bukan penyerahan | Product/domain + pharmacy owner | `open`; `BLOCKING` untuk copy resep dan untuk `PHA-DEC-059` | `PHA-DEC-058`, 3 September 2026 |
...
| Kapabilitas | Sebab |
|---|---|
| Copy resep | `PHA-DEC-058`; menunggu `PHA-OQ-021`, `PHA-OQ-022`, `PHA-OQ-023` |
| Prescription Item Dispensing History | `PHA-DEC-059`; backlog, prasyarat copy resep |
