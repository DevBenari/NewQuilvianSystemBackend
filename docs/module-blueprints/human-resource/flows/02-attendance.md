...
Jenis pengecualian yang paling dekat pada kosakata yang sudah ada adalah `ScheduleMismatch`.
`[EXISTING]` — apakah nilai itu memang dipakai untuk kasus ini, atau perlu nilai tersendiri,
belum ditetapkan. `[OPEN]` — lihat bagian 15
...
| `ScheduleMismatch` | Kehadiran tidak cocok dengan jadwal | `[EXISTING]` |
...
| Koreksi kehadiran | **Perlu.** Status koreksi mengenal `UnderReview`, `Approved`, `PartiallyApproved`, `Rejected` | `[EXISTING]` |
| Penutupan periode | Ada pratinjau sebelum tutup lewat `close-preview` | `[EXISTING]` |
| Pembukaan kembali periode | Ada endpoint `reopen`; siapa yang berwenang belum ditetapkan | `[EXISTING]` untuk endpointnya, `[OPEN]` untuk kewenangannya |
...
| Dari | Tindakan | Ke | Provenance |
| --- | --- | --- | --- |
| `Open` | Mulai tutup | `Closing` | `[EXISTING]` |
| `Closing` | Selesai tutup | `Closed` | `[EXISTING]` |
| `Closed` | Buka kembali | `Reopened` | `[EXISTING]` |
| `Reopened` | Tutup lagi | `Closing` lalu `Closed` | `[EXISTING]` |
| `Open` atau `Closing` | Batalkan | `Cancelled` | `[EXISTING]` |
...
**Transisi yang tidak sah:** `Applied` tidak dapat kembali ke status mana pun. Koreksi yang sudah
diterapkan hanya dapat diperbaiki lewat permohonan koreksi baru. `[EXISTING]` — disimpulkan dari
ketiadaan endpoint pembatalan setelah `Applied`; perlu konfirmasi. `[OPEN]`
...
| Pembukaan kembali periode tercatat | `[EXISTING]` untuk statusnya; pelakunya perlu dikonfirmasi `[OPEN]` |
...
| `HRD-Q-22` | **Baru.** Jenis pengecualian apa yang dipakai untuk aktivitas dokter di luar jadwal kerja? `ScheduleMismatch` sudah ada, tetapi apakah memang untuk kasus ini, atau perlu nilai tersendiri | Memblokir desain final `HRD-DEC-013` |
| `HRD-Q-23` | **Baru.** Siapa yang berwenang membuka kembali periode yang sudah ditutup? Endpoint `reopen` ada, kewenangannya belum ditetapkan | Memblokir desain final penutupan periode |
| `HRD-Q-24` | **Baru.** Apakah koreksi yang sudah `Applied` benar-benar tidak dapat dibatalkan, dan hanya dapat diperbaiki lewat permohonan baru? | Memblokir tabel transisi final |
...
| `AC-F02-05` | Periode tidak dapat ditutup bila masih ada pengecualian yang belum selesai | Tutup periode yang masih punya pengecualian `Open`; ditolak sesuai aturan backend |
| `AC-F02-06` | Pembukaan kembali periode tercatat | Buka kembali periode; status menjadi `Reopened` dan pelakunya tersimpan |
