...
| Corporate / Finance | FinanceManagement / Finance | BUSINESS DOMAIN | Fin | ACTIVE |
| Corporate / Finance | FinanceManagement / BillingIntake / Billing Intake | BUSINESS DOMAIN / MODULE | Fin | ACTIVE |
| Corporate / Finance | FinanceManagement / Receivable / Piutang | BUSINESS DOMAIN / MODULE | Fin | ACTIVE |
| Corporate / Finance | FinanceManagement / Collection / Penerimaan | BUSINESS DOMAIN / MODULE | Fin | ACTIVE |
| Corporate / Finance | FinanceManagement / Payable / Utang | BUSINESS DOMAIN / MODULE | Fin | ACTIVE |
| Corporate / Finance | FinanceManagement / CashManagement / Cash Management | BUSINESS DOMAIN / MODULE | Fin | ACTIVE |
| Corporate / Finance | FinanceManagement / AccountingIntegration / Accounting Integration | BUSINESS DOMAIN / MODULE | Fin | ACTIVE |
| Administrator / HealthServices / Corporate | Master / Reference / MasterData | BUSINESS DOMAIN / MASTER / REFERENCE | Mst | ACTIVE |
...
| HealthServices | NutritionManagement / Nutrition | BUSINESS DOMAIN / MODULE | Gzi | ACTIVE |
...
| HealthServices | HemodialysisManagement / Hemodialysis | BUSINESS DOMAIN / MODULE | Hmd | ACTIVE |
...
| 2026-09-11 | NutritionManagement / `Gz` -> `Gzi` | Prefix modul Gizi diubah dari `Gz` menjadi `Gzi`; lifecycle tetap `ACTIVE` | Ikbal Yuliyanto, pemilik keputusan modul Gizi, sesi 2026-09-11. Prefix dua huruf `Gz` diganti menjadi tiga huruf `Gzi` supaya sepadan dengan seluruh prefix lain pada registry ini, yang semuanya tiga huruf. Mencakup rename seluruh entity operasional `Gz*` menjadi `Gzi*` beserta tabel fisiknya sebagai LEGACY MIGRATION (QBE-NAM-003): sepuluh tabel, 28 baris data, sembilan foreign key yang seluruhnya internal, nol view/function/trigger. Wewenangnya mencakup source dan pembuatan migration; eksekusi database di luar dev pemilik dan deployment tetap merupakan wewenang terpisah. |
...
| 2026-09-17 | FinanceManagement / `Fin` dan MasterData / `Mst` | `Area` Corporate ditambahkan ke baris `Fin` dan `Mst`; `Owner` baris `Fin` bertambah alias `FinanceManagement` | Refactoring Bounded Context DDD kas kecil (Petty Cash). Memindahkan entity operasional anggaran dan mutasi kas kecil (`FinPettyCashBudget`, `FinPettyCashBudgetMovement`) ke `Areas/Corporate/FinanceManagement/PettyCash` serta data induk kategori ke `Areas/Corporate/FinanceManagement/MasterData`, dengan antarmuka loket kasir tetap berada di BillingManagement sebagai klien operasional. Mencabut penghalang QBE-MOD-002 atas penempatan model persisted di bawah `Corporate/FinanceManagement`. |
| 2026-09-21 | FinanceManagement / `Fin` — enam submodul baru | Baris baru — pendaftaran eksplisit enam folder submodul `BillingIntake`, `Receivable`, `Collection`, `Payable`, `CashManagement`, `AccountingIntegration`, seluruhnya prefix `Fin`, lifecycle langsung `ACTIVE` | Yasmin, Product/Domain Owner Finance, blueprint `FIN-BP-001` revisi 2 keputusan `FIN-DES-002`, disetujui 20 September 2026. Task roadmap `BE-FIN-001` (`docs/module-blueprints/finance-management/roadmap/01-backend-roadmap.md`), prasyarat `QBE-MOD-003` sebelum file model pertama Finance ditulis. Baris `Corporate / Finance \| FinanceManagement / Finance \| Fin \| ACTIVE` yang sudah ada (catatan 2026-09-17) sesungguhnya sudah mencakup keenam folder ini secara mekanis lewat `Resolve-RegistryOwnership` — persis preseden `PC-OQ-003` pada `BillingManagement` (lihat `docs/module-blueprints/billing-kasir/task/report/backend/BE-BKC-033.md` §3.4): submodule yang belum terdaftar tersendiri tetap tercakup owner Module-nya selama tidak ada folder lain yang bersaing pada kedalaman token yang sama. Baris ini ditambahkan untuk memenuhi `FIN-DES-002` secara eksplisit (bukan sekadar mengandalkan cakupan implisit), mendokumentasikan nama folder sebenarnya per submodule sesuai *Prosedur pendaftaran modul/prefix baru* langkah 4, dan memberi setiap submodule kedalaman pencocokan tersendiri pada checker. Tidak ada prefix baru: seluruhnya memakai `Fin` yang sudah `ACTIVE`. `MasterData/` sengaja tidak ditambahkan di sini karena bukan bagian dari enam folder `FIN-DES-002` dan sudah tercakup baris `Mst` (`Master / Reference / MasterData`) sejak catatan 2026-09-04. Wewenang ini penamaan dan kepemilikan submodule saja; tidak memberi wewenang implementasi, migration, database, maupun deployment — keduanya tetap menunggu task backend terpisah (`BE-FIN-002` dan seterusnya). |
| 2026-09-22 | HemodialysisManagement / Hemodialysis / `Hmd` | Baris baru — pendaftaran Area `HealthServices`, pemilik `HemodialysisManagement / Hemodialysis`, prefix `Hmd` = *Hemodialysis*, lifecycle langsung **`ACTIVE`** | Muhammad Hamzah, pemilik modul sekaligus pemilik registry (`HMD-FACT-019`), blueprint `HMD-BP-001` revisi 1 keputusan `HMD-DEC-007` yang disetujui 18 September 2026. **Pelaksanaan persetujuan yang sudah ada, bukan persetujuan baru** — tindakan lanjutan 1–3 `HMD-DEC-007` pada `docs/module-blueprints/hemodialisa/00-interview-decisions.md`, diterapkan atas instruksi pemilik pada sesi 22 September 2026 sebagai prasyarat `BE-HMD-01`. Baris identik ditambahkan pada salinan canonical suite skill (tindakan 4) supaya tidak lahir selisih seperti `ACC-DEP-007`; commit dan push kedua repository (tindakan 5) dijalankan pemilik. **Lifecycle langsung `ACTIVE`** mengikuti bunyi `HMD-DEC-007` dan preseden `Num`: `Resolve-RegistryOwnership` hanya menerima token lifecycle `active` dan `legacy`. Entity master milik modul ini — mesin, station, butir checklist, butir kesiapan, dan pengaturan unit — memakai prefix `Hmd`, bukan `Mst`, karena hanya bermakna di unit cuci darah (`02-backend-architecture.md` bagian 10 butir 1). Mencabut penghalang QBE-MOD-002 atas 22 entity operasional `Hmd*` pada `Areas/HealthServices/HemodialysisManagement/`. Persetujuan registry ini penamaan dan kepemilikan; wewenang source, pembuatan migration, dan penerapannya ke database dev pribadi pemilik diberikan terpisah pada sesi yang sama. Deployment dan database bersama tetap merupakan wewenang terpisah. |
