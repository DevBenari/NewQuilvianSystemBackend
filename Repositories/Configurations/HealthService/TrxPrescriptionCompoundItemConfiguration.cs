...
{
    public class TrxPrescriptionCompoundItemConfiguration : IEntityTypeConfiguration<TrxPrescriptionCompoundItem>
    {
        public void Configure(EntityTypeBuilder<TrxPrescriptionCompoundItem> entity)
        {
            entity.ToTable("TrxPrescriptionCompoundItem", "public");
            entity.HasKey(x => x.Id);

            entity.Property(x => x.PrescriptionCompoundId).IsRequired();
            ...
            entity.Property(x => x.AmountPerPackage).HasColumnType("numeric(18,4)").HasDefaultValue(1);
            entity.Property(x => x.TotalQuantity).HasColumnType("numeric(18,4)").HasDefaultValue(1);
            entity.Property(x => x.QuantityUnitNameSnapshot).HasMaxLength(100);
            entity.Property(x => x.QuantityUnitSymbolSnapshot).HasMaxLength(30);
            entity.Property(x => x.IngredientInstruction).HasMaxLength(500);
            ...
            entity.HasOne(x => x.PrescriptionCompound)
                .WithMany(x => x.Items)
            ...
            entity.HasOne(x => x.QuantityUnitMeasurement).WithMany().HasForeignKey(x => x.QuantityUnitMeasurementId).OnDelete(DeleteBehavior.Restrict);
            ...
            entity.HasIndex(x => new { x.PrescriptionCompoundId, x.SortOrder, x.IsDelete });
            entity.HasIndex(x => new { x.PrescriptionCompoundId, x.IsNeedApproval, x.IsApproved, x.IsDelete });
        }
    }
}
