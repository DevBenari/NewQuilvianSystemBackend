...
{
    public class TrxTemporaryAssignmentConfiguration : IEntityTypeConfiguration<TrxTemporaryAssignment>
    {
        public void Configure(EntityTypeBuilder<TrxTemporaryAssignment> builder)
        {
            builder.ToTable("TrxTemporaryAssignment", "public");
            ...
            builder.Property(x => x.AssignmentNumber).HasMaxLength(50).IsRequired();
            builder.Property(x => x.AssignmentStatus).HasMaxLength(50).IsRequired();
            builder.Property(x => x.StartDate).HasColumnType("date");
            builder.Property(x => x.EndDate).HasColumnType("date");
            builder.Property(x => x.ActualReturnDate).HasColumnType("date");
            ...
            builder.HasIndex(x => x.AssignmentNumber).IsUnique().HasFilter("\"IsDelete\" = false");
            builder.HasIndex(x => new { x.WorkforceProfileId, x.StartDate, x.EndDate, x.AssignmentStatus });
            builder.HasCheckConstraint("CK_TrxTemporaryAssignment_DateRange", "\"EndDate\" >= \"StartDate\"");
        }
    }
}
