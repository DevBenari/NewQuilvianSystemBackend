...
namespace QuilvianSystemBackend.Repositories.Configurations.Corporate.HumanResource.MasterData.CompetencyAndCredential
{
    public class MstPositionCompetencyRequirementConfiguration : IEntityTypeConfiguration<MstPositionCompetencyRequirement>
    {
        public void Configure(EntityTypeBuilder<MstPositionCompetencyRequirement> entity)
        {
            ...
            entity.HasIndex(x => new { x.PositionId, x.CompetencyId })
                .IsUnique()
                .HasFilter("\"IsDelete\" = false");
            entity.HasIndex(x => new { x.PositionId, x.IsRequired, x.IsActive, x.IsDelete });
            entity.HasIndex(x => new { x.CompetencyId, x.IsActive, x.IsDelete });
            entity.HasIndex(x => new { x.MinimumLevel, x.IsCertificationRequired, x.IsTrainingRequired });
        }
    }
}
