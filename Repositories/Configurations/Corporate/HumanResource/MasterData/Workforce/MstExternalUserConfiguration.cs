...
namespace QuilvianSystemBackend.Repositories.Configurations.Corporate.HumanResource.MasterData.Workforce
{
    public class MstExternalUserConfiguration : IEntityTypeConfiguration<MstExternalUser>
    {
        public void Configure(EntityTypeBuilder<MstExternalUser> entity)
        {
            ...
            entity.Property(x => x.ExternalCode).HasMaxLength(50).IsRequired();
            ...
            entity.Property(x => x.CompanyCode).HasMaxLength(100);
            ...
            entity.Property(x => x.BusinessLicenseNumber).HasMaxLength(100);
            ...
            entity.Property(x => x.AccessStartDate).HasColumnType("timestamp with time zone").IsRequired(false);
            entity.Property(x => x.AccessEndDate).HasColumnType("timestamp with time zone").IsRequired(false);
            entity.Property(x => x.AccessPurpose).HasMaxLength(250);
            ...
            entity.HasOne(x => x.JobFamily).WithMany().HasForeignKey(x => x.JobFamilyId).OnDelete(DeleteBehavior.Restrict);
            ...
            entity.HasIndex(x => new { x.WorkforceTypeId, x.EmployeeCategoryId, x.EmploymentTypeId, x.ExternalUserStatus, x.IsActive, x.IsDelete });
            entity.HasIndex(x => new { x.ContractTypeId, x.WorkerSourceId });
            entity.HasIndex(x => new { x.ProfessionId, x.JobFamilyId });
            ...
            entity.HasIndex(x => new { x.ContractStartDate, x.ContractEndDate, x.AccessStartDate, x.AccessEndDate });
