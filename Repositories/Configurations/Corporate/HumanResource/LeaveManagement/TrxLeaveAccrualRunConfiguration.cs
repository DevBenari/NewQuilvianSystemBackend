...
{
    public class TrxLeaveAccrualRunConfiguration
        : IEntityTypeConfiguration<TrxLeaveAccrualRun>
    {
        public void Configure(EntityTypeBuilder<TrxLeaveAccrualRun> entity)
        {
            entity.ToTable("TrxLeaveAccrualRun", "public");
            entity.HasKey(x => x.Id);

            entity.Property(x => x.RunNumber).HasMaxLength(50).IsRequired();
            entity.Property(x => x.RunMode)
                .HasMaxLength(30)
                .HasDefaultValue(LeaveValueConstants.BatchRunMode.Scheduled)
                .IsRequired();
            ...
            entity.Property(x => x.TotalCalculatedDays).HasPrecision(18, 4).HasDefaultValue(0);
            entity.Property(x => x.TotalPostedDays).HasPrecision(18, 4).HasDefaultValue(0);
            ...
            entity.HasIndex(x => new
                {
                    ...
                    x.LeaveTypeId,
                    x.LeaveEntitlementPolicyId,
                    x.ScheduledAccrualDate,
                    x.RunMode
                });

            entity.HasIndex(x => new { x.RunStatus, x.ScheduledAccrualDate, x.IsActive, x.IsDelete });
            entity.HasIndex(x => new { x.LegalEntityId, x.HospitalSiteId, x.OrganizationUnitId, x.DepartmentId });
            entity.HasIndex(x => x.CorrelationId);
        }

        private static void ConfigureAuditFields(EntityTypeBuilder<TrxLeaveAccrualRun> entity)
        {
