...
{
    public class TrxLeaveRequestApprovalConfiguration : IEntityTypeConfiguration<TrxLeaveRequestApproval>
    {
        public void Configure(EntityTypeBuilder<TrxLeaveRequestApproval> entity)
        {
            entity.ToTable("TrxLeaveRequestApproval", "public");
            ...
            entity.HasOne(x => x.LeaveRequest).WithMany(x => x.Approvals).HasForeignKey(x => x.LeaveRequestId).OnDelete(DeleteBehavior.Restrict);
            ...
            entity.HasIndex(x => new { x.LeaveRequestId, x.StepOrder }).IsUnique().HasFilter("\"IsDelete\" = false");
