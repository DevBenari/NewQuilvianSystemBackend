...
{
    public class TrxAttendanceDailySegmentConfiguration : IEntityTypeConfiguration<TrxAttendanceDailySegment>
    {
        public void Configure(EntityTypeBuilder<TrxAttendanceDailySegment> builder)
        {
            builder.ToTable("TrxAttendanceDailySegment", "public");
            ...
            builder.Property(x => x.SegmentType).HasMaxLength(30).IsRequired();
            builder.Property(x => x.SegmentSource).HasMaxLength(30);
            ...
            builder.Property(x => x.SegmentStatus).HasMaxLength(30);
            builder.Property(x => x.Notes).HasMaxLength(500);
            builder.Property(x => x.IsActive).HasDefaultValue(true);

            builder.HasOne(x => x.AttendanceDaily).WithMany(x => x.Segments).HasForeignKey(x => x.AttendanceDailyId).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(x => x.StartRawLog).WithMany().HasForeignKey(x => x.StartRawLogId).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(x => x.EndRawLog).WithMany().HasForeignKey(x => x.EndRawLogId).OnDelete(DeleteBehavior.Restrict);

            builder.HasIndex(x => new { x.AttendanceDailyId, x.SegmentOrder }).IsUnique().HasFilter("\"IsDelete\" = false");
