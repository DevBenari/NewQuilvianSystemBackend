...
{
    public class TrxAttendanceRawLogConfiguration : IEntityTypeConfiguration<TrxAttendanceRawLog>
    {
        public void Configure(EntityTypeBuilder<TrxAttendanceRawLog> builder)
        {
            builder.ToTable("TrxAttendanceRawLog", "public");
            ...
            builder.Property(x => x.EventAt).HasColumnType("timestamp with time zone");
            builder.Property(x => x.ReceivedAt).HasColumnType("timestamp with time zone").HasDefaultValueSql("CURRENT_TIMESTAMP");
            ...
            builder.HasOne(x => x.AttendanceDevice).WithMany().HasForeignKey(x => x.AttendanceDeviceId).OnDelete(DeleteBehavior.Restrict);
            ...
            builder.HasOne(x => x.ProcessedAttendance).WithMany().HasForeignKey(x => x.ProcessedAttendanceId).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(x => x.ProcessedAttendanceDaily).WithMany(x => x.RawLogs).HasForeignKey(x => x.ProcessedAttendanceDailyId).OnDelete(DeleteBehavior.Restrict);

            builder.HasIndex(x => new { x.AttendanceDeviceId, x.ExternalLogId }).IsUnique().HasFilter("\"ExternalLogId\" IS NOT NULL AND \"IsDelete\" = false");
            builder.HasIndex(x => x.EventHash).IsUnique().HasFilter("\"EventHash\" IS NOT NULL AND \"IsDelete\" = false");
