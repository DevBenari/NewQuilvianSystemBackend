...
namespace QuilvianSystemBackend.Repositories.Configurations.Corporate.HumanResource.LifecycleManagement
{
    ...
    {
        public void Configure(EntityTypeBuilder<WfpOnboardingTask> builder)
        {
            builder.ToTable("WfpOnboardingTask", "public");
            ...
            builder.HasOne(x => x.OnboardingChecklist).WithMany(x => x.Tasks).HasForeignKey(x => x.OnboardingChecklistId).OnDelete(DeleteBehavior.Restrict);
            ...
            builder.HasIndex(x => new { x.OnboardingChecklistId, x.TaskCode }).IsUnique().HasFilter("\"IsDelete\" = false");
