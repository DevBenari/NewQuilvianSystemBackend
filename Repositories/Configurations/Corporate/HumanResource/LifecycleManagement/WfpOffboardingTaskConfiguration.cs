...
namespace QuilvianSystemBackend.Repositories.Configurations.Corporate.HumanResource.LifecycleManagement
{
    ...
    {
        public void Configure(EntityTypeBuilder<WfpOffboardingTask> builder)
        {
            builder.ToTable("WfpOffboardingTask", "public");
            ...
            builder.HasOne(x => x.OffboardingChecklist).WithMany(x => x.Tasks).HasForeignKey(x => x.OffboardingChecklistId).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(x => x.OffboardingTemplateTask).WithMany().HasForeignKey(x => x.OffboardingTemplateTaskId).OnDelete(DeleteBehavior.Restrict);
            ...
            builder.HasIndex(x => new { x.OffboardingChecklistId, x.TaskCode }).IsUnique().HasFilter("\"IsDelete\" = false");
