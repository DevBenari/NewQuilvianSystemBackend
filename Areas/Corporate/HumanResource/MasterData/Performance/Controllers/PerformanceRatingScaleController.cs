...
namespace QuilvianSystemBackend.Areas.Corporate.HumanResource.MasterData.Performance.Controllers
{
    ...
    public class PerformanceRatingScaleController : ControllerBase
    {
        ...
        {
            DefaultFilter = new(), CustomPeriods = BuildPeriodOptions(), ScaleTypeOptions = Types.Select(x => new PerformanceStringOptionResponse
            {
                Value = x, Label = x
            }
        ...
        [AccessAction("Read", "Read Performance Rating Scale", AccessType = AccessTypes.Read, SortOrder = 1)]
        [AccessPermission("PerformanceRatingScale", "Read")]
        public async Task<IActionResult> List([FromQuery] DateTime? startDate, [FromQuery] DateTime? endDate, [FromQuery] string? customPeriod, [FromQuery] string? scaleType, [FromQuery] bool? isDefault, [FromQuery] bool? isActive, [FromQuery] string? search, [FromQuery] string? sortBy = "scaleName", [FromQuery] string? sortDirection = "asc", [FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 25, CancellationToken ct = default)
        {
