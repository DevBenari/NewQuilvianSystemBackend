...
using ResponseDoctorPagedResult =
    QuilvianSystemBackend.Responses.PagedResult<
        QuilvianSystemBackend.Areas.Corporate.HumanResource.MasterData.Workforce.DTOs.DoctorResponse>;
...
namespace QuilvianSystemBackend.Areas.Corporate.HumanResource.MasterData.Workforce.Controllers
{
    ...
    public class DoctorController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetFilterMetadataForAdmin()
        {
            var result = new DoctorFilterMetadataResponse
            {
                ...
                CredentialingStatusOptions = BuildEnumOptions<CredentialingStatus>(),
                ClinicalPrivilegeStatusOptions = BuildEnumOptions<ClinicalPrivilegeStatus>(),
            ...
            await PopulateDoctorMasterOptionsAsync(result);
        ...
        public async Task<IActionResult> GetSummaryForAdmin()
        {
            ...
            var result = new DoctorSummaryResponse
            {
                ...
                CredentialingPendingDoctor = await doctorQuery.CountAsync(x =>
                    x.CredentialingStatus == CredentialingStatus.Pending ||
                    x.CredentialingStatus == CredentialingStatus.UnderReview),
                PrivilegeSuspendedDoctor = await doctorQuery.CountAsync(x =>
                    x.ClinicalPrivilegeStatus == ClinicalPrivilegeStatus.Suspended)
            };
            ...
            [FromQuery] int pageSize = 25)
        {
            ...
            var items = await ApplyDoctorSorting(query, sortBy, sortDirection)
                ...
                .Select(x => new DoctorResponse
                {
                    ...
                    WorkforceTypeCode = x.WorkforceType != null ? x.WorkforceType.WorkforceTypeCode : string.Empty,
                    WorkforceTypeName = x.WorkforceType != null ? x.WorkforceType.WorkforceTypeName : string.Empty,
                    EmployeeCategoryId = x.EmployeeCategoryId,
                    EmployeeCategoryCode = x.EmployeeCategory != null ? x.EmployeeCategory.EmployeeCategoryCode : string.Empty,
                    EmployeeCategoryName = x.EmployeeCategory != null ? x.EmployeeCategory.EmployeeCategoryName : string.Empty,
                    EmploymentTypeId = x.EmploymentTypeId,
                    EmploymentTypeCode = x.EmploymentType != null ? x.EmploymentType.EmploymentTypeCode : string.Empty,
                    EmploymentTypeName = x.EmploymentType != null ? x.EmploymentType.EmploymentTypeName : string.Empty,
                    EmploymentStatusId = x.EmploymentStatusId,
                    EmploymentStatusCode = x.EmploymentStatus != null ? x.EmploymentStatus.EmploymentStatusCode : string.Empty,
                    EmploymentStatusName = x.EmploymentStatus != null ? x.EmploymentStatus.EmploymentStatusName : string.Empty,
                    ...
                    CredentialingStatus = x.CredentialingStatus,
                    ClinicalPrivilegeStatus = x.ClinicalPrivilegeStatus,
        ...
        public async Task<IActionResult> CreateDoctorForAdmin([FromBody] CreateDoctorRequest request)
        {
            ...
            var masterValidation = await ValidateDoctorMasterReferencesAsync(
                workforceTypeId: request.WorkforceTypeId,
                employeeCategoryId: request.EmployeeCategoryId,
                employmentTypeId: request.EmploymentTypeId,
                employmentStatusId: request.EmploymentStatusId,
                contractTypeId: request.ContractTypeId,
                workerSourceId: request.WorkerSourceId,
            ...
            if (!masterValidation.IsValid)
            {
                return BadRequest(ApiResponse<object>.Fail(
                    StatusCodes.Status400BadRequest,
                    masterValidation.ErrorMessage ?? "Referensi master doctor tidak valid."
                ));
            }
            ...
            try
            {
                ...
                var entity = new MstDoctor
                {
                    ...
                    ProfessionId = request.ProfessionId,
                    SpecializationId = NormalizeNullableGuid(request.SpecializationId),
                    PracticeType = request.PracticeType,
                    CredentialingStatus = request.CredentialingStatus,
                    ClinicalPrivilegeStatus = request.ClinicalPrivilegeStatus,
                ...
                if (request.CreateLoginAccount)
                {
                    ...
                    {
                        await SyncUserOrganizationProjectionAsync(entity.WorkforceProfileId, actorUserId);
            ...
            [FromBody] UpdateDoctorRequest request)
        {
            ...
            entity.SpecializationId = NormalizeNullableGuid(request.SpecializationId);
            entity.PracticeType = request.PracticeType;
            entity.CredentialingStatus = request.CredentialingStatus;
            entity.ClinicalPrivilegeStatus = request.ClinicalPrivilegeStatus;
            ...
            [FromBody] UpdateDoctorStatusRequest request)
        {
            ...
            var employmentStatusExists = await _dbContext.Set<MstEmploymentStatus>()
                .AsNoTracking()
                .AnyAsync(x =>
                    x.Id == request.EmploymentStatusId &&
                    x.IsActive &&
                    !x.IsDelete);
            ...
            if (!employmentStatusExists)
            {
                return BadRequest(ApiResponse<object>.Fail(
                    StatusCodes.Status400BadRequest,
                    "Employment status tidak ditemukan atau sudah tidak aktif."
                ));
            }
    ...
    [FromBody] UpdateDoctorUserGeolocationBypassRequest request)
        ...
        private async Task<(bool IsValid, string? ErrorMessage)> ValidateDoctorMasterReferencesAsync(
            Guid workforceTypeId,
        ...
        {
            if (!await _dbContext.Set<MstWorkforceType>().AsNoTracking()
                .AnyAsync(x => x.Id == workforceTypeId && x.IsActive && !x.IsDelete))
                return (false, "Jenis tenaga kerja tidak ditemukan atau sudah tidak aktif.");

            var category = await _dbContext.Set<MstEmployeeCategory>().AsNoTracking()
                .Where(x => x.Id == employeeCategoryId && x.IsActive && !x.IsDelete)
                .Select(x => new { x.WorkforceTypeId })
                .FirstOrDefaultAsync();

            if (category == null)
                return (false, "Kategori tenaga medis tidak ditemukan atau sudah tidak aktif.");

            if (category.WorkforceTypeId.HasValue && category.WorkforceTypeId.Value != workforceTypeId)
                return (false, "Kategori tenaga medis tidak sesuai dengan jenis tenaga kerja yang dipilih.");

            var employmentType = await _dbContext.Set<MstEmploymentType>().AsNoTracking()
                .Where(x => x.Id == employmentTypeId && x.IsActive && !x.IsDelete)
            ...
            if (employmentType.RequiresContractEndDate && !contractEndDate.HasValue)
                return (false, "Tanggal akhir kontrak wajib diisi untuk jenis kepegawaian yang dipilih.");

            if (!await _dbContext.Set<MstEmploymentStatus>().AsNoTracking()
                .AnyAsync(x => x.Id == employmentStatusId && x.IsActive && !x.IsDelete))
                return (false, "Status kepegawaian tidak ditemukan atau sudah tidak aktif.");

            if (contractTypeId.HasValue && contractTypeId.Value != Guid.Empty)
            {
                var contractType = await _dbContext.Set<MstContractType>().AsNoTracking()
                    .Where(x => x.Id == contractTypeId.Value && x.IsActive && !x.IsDelete)
                ...
                if (contractType.RequiresEndDate && !contractEndDate.HasValue)
                    return (false, "Tanggal akhir kontrak wajib diisi untuk jenis kontrak yang dipilih.");
            }

            if (workerSourceId.HasValue && workerSourceId.Value != Guid.Empty &&
                !await _dbContext.Set<MstWorkerSource>().AsNoTracking()
                    .AnyAsync(x => x.Id == workerSourceId.Value && x.IsActive && !x.IsDelete))
                return (false, "Sumber tenaga kerja tidak ditemukan atau sudah tidak aktif.");

            if (!await _dbContext.Set<MstProfession>().AsNoTracking()
                .AnyAsync(x => x.Id == professionId && x.IsActive && !x.IsDelete))
                return (false, "Profesi tidak ditemukan atau sudah tidak aktif.");

            if (specializationId.HasValue && specializationId.Value != Guid.Empty)
            {
                var specializationValid = await _dbContext.Set<MstSpecialization>().AsNoTracking()
                    .AnyAsync(x =>
                        x.Id == specializationId.Value &&
                        x.ProfessionId == professionId &&
                        x.IsActive &&
                        !x.IsDelete);

                if (!specializationValid)
                    return (false, "Spesialisasi tidak ditemukan, tidak aktif, atau tidak sesuai dengan profesi yang dipilih.");
            }
        ...
        private static List<DoctorFormFieldMetadataResponse> BuildDoctorFieldMetadata(bool isUpdate)
        {
            return new List<DoctorFormFieldMetadataResponse>
            {
                ...
                new()
                {
                    ...
                    Label = "Kategori Tenaga Medis",
                    Section = "Doctor Profile",
                    InputType = "select",
                    IsRequiredOnCreate = true,
                    IsRequiredOnUpdate = true,
                    RequiredType = "Required",
                    DependsOn = "workforceTypeId",
                    OptionsSource = "employeeCategoryOptions",
                    SortOrder = 402
                },
                new()
                {
                ...
                new()
                {
                    ...
                    IsRequiredOnUpdate = true,
                    RequiredType = "Required",
                    OptionsSource = "employmentStatusOptions",
                    SortOrder = 404
                },
                new()
                {
                    ...
                    IsRequiredOnUpdate = false,
                    RequiredType = "Optional",
                    OptionsSource = "contractTypeOptions",
                    SortOrder = 405
                },
                new()
                {
                    Name = "workerSourceId",
                    Label = "Sumber Tenaga Kerja",
                    Section = "Doctor Profile",
                    InputType = "select",
                    IsRequiredOnCreate = false,
                    IsRequiredOnUpdate = false,
                    RequiredType = "Optional",
                    OptionsSource = "workerSourceOptions",
                    SortOrder = 406
                },
                new()
                {
                    ...
                    IsRequiredOnUpdate = true,
                    RequiredType = "Required",
                    OptionsSource = "professionOptions",
                    SortOrder = 407
                },
                new()
                {
                    ...
                    OptionsSource = "specializationOptions",
                    SortOrder = 408
                },
                new()
                {
                ...
                new()
                {
                    ...
                    Label = "Status Credentialing",
                    Section = "Credentialing",
                    InputType = "select",
                    IsRequiredOnCreate = false,
                    IsRequiredOnUpdate = false,
                    RequiredType = "Optional",
                    OptionsSource = "credentialingStatusOptions",
                    SortOrder = 410
                },
                new()
                {
                    Name = "clinicalPrivilegeStatus",
                    Label = "Status Kewenangan Klinis",
                    Section = "Credentialing",
                    InputType = "select",
                    IsRequiredOnCreate = false,
                    IsRequiredOnUpdate = false,
                    RequiredType = "Optional",
                    OptionsSource = "clinicalPrivilegeStatusOptions",
                    SortOrder = 411
                },
                new()
                {
        ...
        private async Task PopulateDoctorMasterOptionsAsync(DoctorFilterMetadataResponse result)
        {
            result.WorkforceTypeOptions = await _dbContext.Set<MstWorkforceType>()
                ...
                {
                    Value = x.Id, Code = x.WorkforceTypeCode, Name = x.WorkforceTypeName, Label = x.WorkforceTypeName
                }).ToListAsync();

            result.EmployeeCategoryOptions = await _dbContext.Set<MstEmployeeCategory>()
                .AsNoTracking().Where(x => x.IsActive && !x.IsDelete)
                .OrderBy(x => x.SortOrder).ThenBy(x => x.EmployeeCategoryName)
                .Select(x => new DoctorMasterOptionResponse
                {
                    Value = x.Id, Code = x.EmployeeCategoryCode, Name = x.EmployeeCategoryName,
                    Label = x.EmployeeCategoryName, ParentId = x.WorkforceTypeId
                }).ToListAsync();

            result.EmploymentTypeOptions = await _dbContext.Set<MstEmploymentType>()
                .AsNoTracking().Where(x => x.IsActive && !x.IsDelete)
                .OrderBy(x => x.SortOrder).ThenBy(x => x.EmploymentTypeName)
                .Select(x => new DoctorMasterOptionResponse
                {
                    Value = x.Id, Code = x.EmploymentTypeCode, Name = x.EmploymentTypeName, Label = x.EmploymentTypeName
                }).ToListAsync();

            result.EmploymentStatusOptions = await _dbContext.Set<MstEmploymentStatus>()
                .AsNoTracking().Where(x => x.IsActive && !x.IsDelete)
                .OrderBy(x => x.SortOrder).ThenBy(x => x.EmploymentStatusName)
                .Select(x => new DoctorMasterOptionResponse
                {
                    Value = x.Id, Code = x.EmploymentStatusCode, Name = x.EmploymentStatusName, Label = x.EmploymentStatusName
                }).ToListAsync();

            result.ContractTypeOptions = await _dbContext.Set<MstContractType>()
                .AsNoTracking().Where(x => x.IsActive && !x.IsDelete)
                .OrderBy(x => x.SortOrder).ThenBy(x => x.ContractTypeName)
                .Select(x => new DoctorMasterOptionResponse
                {
                    Value = x.Id, Code = x.ContractTypeCode, Name = x.ContractTypeName, Label = x.ContractTypeName
                }).ToListAsync();

            result.WorkerSourceOptions = await _dbContext.Set<MstWorkerSource>()
                .AsNoTracking().Where(x => x.IsActive && !x.IsDelete)
                .OrderBy(x => x.SortOrder).ThenBy(x => x.WorkerSourceName)
                .Select(x => new DoctorMasterOptionResponse
                {
                    Value = x.Id, Code = x.WorkerSourceCode, Name = x.WorkerSourceName, Label = x.WorkerSourceName
                }).ToListAsync();
            ...
            result.SpecializationOptions = await _dbContext.Set<MstSpecialization>()
                ...
                .Select(x => new DoctorMasterOptionResponse
                {
                    ...
                    Label = x.SpecializationName, ParentId = x.ProfessionId
                }).ToListAsync();
        }
        ...
        private static List<DoctorEnumOptionResponse> BuildDoctorUserTypeOptions()
        {
            return new List<UserType>
                ...
                .Select(x => new DoctorEnumOptionResponse
                {
                    ...
                    Name = x.ToString(),
                    Label = GetEnumDisplayName(x)
                })
                .ToList();
        }

        private static List<DoctorEnumOptionResponse> BuildEnumOptions<TEnum>() where TEnum : struct, Enum
        {
        ...
        private static string GetEnumDisplayName<TEnum>(TEnum value) where TEnum : struct, Enum
        {
            ...
            if (member != null)
            {
                var displayAttribute = Attribute.GetCustomAttribute(
                    member,
            ...
            return BuildEnumLabel(value.ToString());
        }
            ...
            Guid professionId)
        {
            ...
            if (workforceTypeId == Guid.Empty)
                return (false, "Jenis tenaga kerja wajib dipilih.");

            if (employeeCategoryId == Guid.Empty)
                return (false, "Kategori tenaga medis wajib dipilih.");

            if (employmentTypeId == Guid.Empty)
                return (false, "Jenis kepegawaian wajib dipilih.");

            if (employmentStatusId == Guid.Empty)
                return (false, "Status kepegawaian wajib dipilih.");

            if (professionId == Guid.Empty)
                return (false, "Profesi wajib dipilih.");
