...
using ResponseEmployeePagedResult =
    QuilvianSystemBackend.Responses.PagedResult<
        QuilvianSystemBackend.Areas.Corporate.HumanResource.MasterData.Workforce.DTOs.EmployeeResponse>;
...
namespace QuilvianSystemBackend.Areas.Corporate.HumanResource.MasterData.Workforce.Controllers
{
    ...
    public class EmployeeController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetFilterMetadata()
        {
            ...
            await PopulateEmployeeMasterOptionsAsync(result);
        ...
        public async Task<IActionResult> GetSummary()
        {
            ...
            // WfpTransportAllowance versi Payroll Management tidak lagi menggunakan
            // IsEligible dan IsNightTransportEligible.
            // Eligibility ditentukan dari status profile, periode efektif, dan policy aktif.
            var activeTransportProfileQuery = transportProfileQuery
                .Where(x =>
                    x.IsActive &&
                    x.AllowanceStatus == "Active" &&
                    (!x.EffectiveStartDate.HasValue || x.EffectiveStartDate.Value <= operationalDate) &&
                    (!x.EffectiveEndDate.HasValue || x.EffectiveEndDate.Value >= operationalDate));
            ...
            var result = new EmployeeSummaryResponse
            {
                ...
                TransportEligibleEmployee = await employeeQuery.CountAsync(x =>
                    ...
                    activeTransportProfileQuery.Any(t =>
                        ...
                        (
                            t.TransportAllowancePolicyId.HasValue ||
                            t.MonthlyAmount > 0 ||
                ...
                NightTransportEligibleEmployee = await employeeQuery.CountAsync(x =>
                    ...
                    activeTransportProfileQuery.Any(t =>
                        t.WorkforceProfileId == x.WorkforceProfileId &&
                        t.TransportAllowancePolicy != null &&
                        t.TransportAllowancePolicy.IsActive &&
            ...
            [FromQuery] int pageSize = 25)
        {
            ...
            var items = await ApplyEmployeeSorting(query, sortBy, sortDirection)
                ...
                .Select(x => new EmployeeResponse
                {
                    ...
                    JobFamilyCode = x.JobFamily != null ? x.JobFamily.JobFamilyCode : null,
                    JobFamilyName = x.JobFamily != null ? x.JobFamily.JobFamilyName : null,
        ...
        public async Task<IActionResult> CreateEmployee([FromBody] CreateEmployeeRequest request)
        {
            ...
            var masterValidation = await ValidateEmployeeMasterReferencesAsync(
            ...
            if (!masterValidation.IsValid)
            {
                return BadRequest(ApiResponse<object>.Fail(
                    StatusCodes.Status400BadRequest,
                    masterValidation.ErrorMessage ?? "Referensi master employee tidak valid."
                ));
            }
            ...
            try
            {
                ...
                var entity = new MstEmployee
                {
                    ...
                    ProfessionId = NormalizeNullableGuid(request.ProfessionId),
                    JobFamilyId = NormalizeNullableGuid(request.JobFamilyId),
            ...
            [FromBody] UpdateEmployeeRequest request)
        {
            ...
            entity.ProfessionId = NormalizeNullableGuid(request.ProfessionId);
            entity.JobFamilyId = NormalizeNullableGuid(request.JobFamilyId);
        ...
        private async Task<(bool IsValid, string? ErrorMessage)> ValidateEmployeeMasterReferencesAsync(
            ...
            DateTime? contractEndDate)
        {
            ...
            var workforceTypeExists = await _dbContext.Set<MstWorkforceType>()
                .AsNoTracking()
                .AnyAsync(x => x.Id == workforceTypeId && x.IsActive && !x.IsDelete);

            if (!workforceTypeExists)
                return (false, "Jenis tenaga kerja tidak ditemukan atau sudah tidak aktif.");

            if (employeeCategoryId == Guid.Empty)
                return (false, "Kategori pegawai wajib dipilih.");

            var employeeCategory = await _dbContext.Set<MstEmployeeCategory>()
                .AsNoTracking()
            ...
            if (employeeCategory == null)
                return (false, "Kategori pegawai tidak ditemukan atau sudah tidak aktif.");

            if (employeeCategory.WorkforceTypeId.HasValue &&
                employeeCategory.WorkforceTypeId.Value != workforceTypeId)
            {
                return (false, "Kategori pegawai tidak sesuai dengan jenis tenaga kerja yang dipilih.");
            }

            var employmentType = await _dbContext.Set<MstEmploymentType>()
                .AsNoTracking()
            ...
            {
                var contractType = await _dbContext.Set<MstContractType>()
                    .AsNoTracking()
            ...
            if (professionId.HasValue && professionId.Value != Guid.Empty &&
                !await _dbContext.Set<MstProfession>().AsNoTracking()
                    .AnyAsync(x => x.Id == professionId.Value && x.IsActive && !x.IsDelete))
            {
                return (false, "Profesi tidak ditemukan atau sudah tidak aktif.");
            }

            if (jobFamilyId.HasValue && jobFamilyId.Value != Guid.Empty &&
                !await _dbContext.Set<MstJobFamily>().AsNoTracking()
                    .AnyAsync(x => x.Id == jobFamilyId.Value && x.IsActive && !x.IsDelete))
            {
                return (false, "Rumpun pekerjaan tidak ditemukan atau sudah tidak aktif.");
            }
    ...
    Guid actorUserId)
        ...
        private static List<EmployeeFormFieldMetadataResponse> BuildEmployeeFieldMetadata(bool isUpdate)
        {
            return new List<EmployeeFormFieldMetadataResponse>
            {
                ...
                {
                    Name = "jobFamilyId",
                    Label = "Rumpun Pekerjaan",
                    Section = "Employment",
                    InputType = "select",
                    IsRequiredOnCreate = false,
                    IsRequiredOnUpdate = false,
                    RequiredType = "Optional",
                    OptionsSource = "jobFamilyOptions",
                    SortOrder = 408
                },
                new()
                {
        ...
        private async Task PopulateEmployeeMasterOptionsAsync(EmployeeFilterMetadataResponse result)
        {
            result.WorkforceTypeOptions = await _dbContext.Set<MstWorkforceType>()
                .AsNoTracking()
                .Where(x => x.IsActive && !x.IsDelete)
                .OrderBy(x => x.SortOrder)
                .ThenBy(x => x.WorkforceTypeName)
                .Select(x => new EmployeeMasterOptionResponse
                {
                    Value = x.Id,
                    Code = x.WorkforceTypeCode,
                    Name = x.WorkforceTypeName,
                    Label = x.WorkforceTypeName
                })
                .ToListAsync();
            ...
            result.EmployeeCategoryOptions = await _dbContext.Set<MstEmployeeCategory>()
                ...
                .Where(x => x.IsActive && !x.IsDelete)
                .OrderBy(x => x.SortOrder)
                .ThenBy(x => x.EmployeeCategoryName)
                .Select(x => new EmployeeMasterOptionResponse
                {
                    Value = x.Id,
                    Code = x.EmployeeCategoryCode,
                    Name = x.EmployeeCategoryName,
                    Label = x.EmployeeCategoryName,
                    ParentId = x.WorkforceTypeId
                })
                .ToListAsync();
            ...
            result.EmploymentStatusOptions = await _dbContext.Set<MstEmploymentStatus>()
                ...
                .Where(x => x.IsActive && !x.IsDelete)
                .OrderBy(x => x.SortOrder)
                .ThenBy(x => x.EmploymentStatusName)
                .Select(x => new EmployeeMasterOptionResponse
                {
                    Value = x.Id,
                    Code = x.EmploymentStatusCode,
                    Name = x.EmploymentStatusName,
                    Label = x.EmploymentStatusName
                })
                .ToListAsync();
            ...
            result.ProfessionOptions = await _dbContext.Set<MstProfession>()
                ...
                .Where(x => x.IsActive && !x.IsDelete)
                .OrderBy(x => x.SortOrder)
                .ThenBy(x => x.ProfessionName)
                .Select(x => new EmployeeMasterOptionResponse
                {
                    Value = x.Id,
                    Code = x.ProfessionCode,
                    Name = x.ProfessionName,
                    Label = x.ProfessionName
                })
                .ToListAsync();
            ...
            result.JobFamilyOptions = await _dbContext.Set<MstJobFamily>()
                ...
                .Where(x => x.IsActive && !x.IsDelete)
                .OrderBy(x => x.SortOrder)
                .ThenBy(x => x.JobFamilyName)
                .Select(x => new EmployeeMasterOptionResponse
                {
                    Value = x.Id,
                    Code = x.JobFamilyCode,
                    Name = x.JobFamilyName,
                    Label = x.JobFamilyName
                })
                .ToListAsync();
        }

        private static List<EmployeeEnumOptionResponse> BuildEnumOptions<TEnum>() where TEnum : struct, Enum
        {
