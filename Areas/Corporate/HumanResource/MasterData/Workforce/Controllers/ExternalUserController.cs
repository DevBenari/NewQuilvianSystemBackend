...
using ResponseExternalUserPagedResult =
    QuilvianSystemBackend.Responses.PagedResult<
        QuilvianSystemBackend.Areas.Corporate.HumanResource.MasterData.Workforce.DTOs.ExternalUserResponse>;
...
namespace QuilvianSystemBackend.Areas.Corporate.HumanResource.MasterData.Workforce.Controllers
{
    ...
    public class ExternalUserController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetFilterMetadata()
        {
            ...
            await PopulateExternalUserMasterOptionsAsync(result);
            ...
            [FromQuery] int pageSize = 25)
        {
            ...
            var items = await ApplyExternalUserSorting(query, sortBy, sortDirection)
                ...
                .Select(x => new ExternalUserResponse
                {
                    ...
                    EmployeeCategoryCode = x.EmployeeCategory != null ? x.EmployeeCategory.EmployeeCategoryCode : null,
                    EmployeeCategoryName = x.EmployeeCategory != null ? x.EmployeeCategory.EmployeeCategoryName : null,
        ...
        public async Task<IActionResult> CreateExternalUser([FromBody] CreateExternalUserRequest request)
        {
            ...
            var masterValidation = await ValidateExternalUserMasterReferencesAsync(
            ...
            if (!masterValidation.IsValid)
            {
                return BadRequest(ApiResponse<object>.Fail(
                    StatusCodes.Status400BadRequest,
                    masterValidation.ErrorMessage ?? "Referensi master external user tidak valid."
                ));
            }
            ...
            try
            {
                ...
                var entity = new MstExternalUser
                {
                    ...
                    WorkforceTypeId = request.WorkforceTypeId,
                    EmployeeCategoryId = NormalizeNullableGuid(request.EmployeeCategoryId),
                ...
                var response = new ExternalUserCreateResponse
                {
                    ...
                    WorkforceTypeId = entity.WorkforceTypeId,
                    WorkforceTypeName = await _dbContext.Set<MstWorkforceType>()
                        .Where(x => x.Id == entity.WorkforceTypeId)
                        .Select(x => x.WorkforceTypeName)
            ...
            [FromBody] UpdateExternalUserRequest request)
        {
            ...
            entity.EmployeeCategoryId = NormalizeNullableGuid(request.EmployeeCategoryId);
    ...
    [FromBody] UpdateExternalUserAccountGeolocationBypassRequest request)
        ...
        private async Task<(bool IsValid, string? ErrorMessage)> ValidateExternalUserMasterReferencesAsync(
            ...
            DateTime? contractEndDate)
        {
            ...
            if (employeeCategoryId.HasValue && employeeCategoryId.Value != Guid.Empty)
            {
                var categoryValid = await _dbContext.Set<MstEmployeeCategory>().AsNoTracking()
                    .AnyAsync(x =>
                        x.Id == employeeCategoryId.Value &&
                        (!x.WorkforceTypeId.HasValue || x.WorkforceTypeId == workforceTypeId) &&
                        x.IsActive &&
                        !x.IsDelete);

                if (!categoryValid)
                    return (false, "Kategori tenaga kerja tidak ditemukan, tidak aktif, atau tidak sesuai dengan jenis tenaga kerja.");
            }

            if (employmentTypeId.HasValue && employmentTypeId.Value != Guid.Empty)
            {
                var employmentType = await _dbContext.Set<MstEmploymentType>().AsNoTracking()
                    .Where(x => x.Id == employmentTypeId.Value && x.IsActive && !x.IsDelete)
        ...
        private async Task PopulateExternalUserMasterOptionsAsync(ExternalUserFilterMetadataResponse result)
        {
        ...
        private static List<ExternalUserEnumOptionResponse> BuildEnumOptions<TEnum>() where TEnum : struct, Enum
        {
