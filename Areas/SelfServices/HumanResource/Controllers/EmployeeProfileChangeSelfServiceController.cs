...
namespace QuilvianSystemBackend.Areas.SelfServices.HumanResource.Controllers
{
    ...
    public class EmployeeProfileChangeSelfServiceController : ControllerBase
    {
        ...
        [AccessAction(
            "Read",
            "Read My Profile Change",
            Description = "Melihat metadata pengajuan perubahan profil",
        ...
        [AccessAction(
            "Read",
            "Read My Profile Change",
            Description = "Melihat ringkasan perubahan profil milik sendiri",
            AccessType = AccessTypes.Read,
            SortOrder = 1)]
        [AccessPermission("MyProfileChange", "Read")]
        ...
        [AccessAction(
            "Read",
            "Read My Profile Change",
            Description = "Melihat daftar perubahan profil milik sendiri",
            AccessType = AccessTypes.Read,
            SortOrder = 1)]
        [AccessPermission("MyProfileChange", "Read")]
        ...
        [AccessAction(
            "Read",
            "Read My Profile Change",
            Description = "Melihat detail perubahan profil milik sendiri",
            AccessType = AccessTypes.Read,
            SortOrder = 1)]
        [AccessPermission("MyProfileChange", "Read")]
        ...
        [AccessAction(
            "Read",
            "Read My Profile Change Workflow",
            Description = "Melihat workflow perubahan profil milik sendiri",
            AccessType = AccessTypes.Read,
            SortOrder = 1)]
        [AccessPermission("MyProfileChange", "Read")]
            ...
            CancellationToken cancellationToken)
        {
            ...
            return ToActionResult(
                await _workflowIntegrationService.GetWorkflowAsync(
                    id,
                    cancellationToken));
        }
        ...
        [AccessAction(
            "Create",
            "Create My Profile Change",
            Description = "Membuat draft perubahan profil milik sendiri",
            AccessType = AccessTypes.Create,
            SortOrder = 2)]
        [AccessPermission("MyProfileChange", "Create")]
        public async Task<IActionResult> Create(
            [FromBody] CreateEmployeeProfileChangeSelfServiceRequest request,
            CancellationToken cancellationToken)
        {
            var contextResult = await ResolveContextAsync(cancellationToken);
            if (contextResult.Error != null)
            ...
            var domainRequest = new CreateEmployeeProfileChangeRequest
            {
                WorkforceProfileId =
                    contextResult.Context!.WorkforceProfileId!.Value,
                WorkflowDefinitionId = null,
                RequestReasonId = request.RequestReasonId,
                RequestCategory = request.RequestCategory,
                RequestReasonText = request.RequestReasonText,
        ...
        [AccessAction(
            "Update",
            "Update My Profile Change",
            Description = "Mengubah draft atau revisi perubahan profil milik sendiri",
            AccessType = AccessTypes.Update,
            SortOrder = 3)]
        [AccessPermission("MyProfileChange", "Update")]
        public async Task<IActionResult> Update(
            Guid id,
            [FromBody] UpdateEmployeeProfileChangeSelfServiceRequest request,
            CancellationToken cancellationToken)
        {
            var owned = await GetOwnedAsync(
                id,
            ...
            var domainRequest = new UpdateEmployeeProfileChangeRequest
            {
                WorkflowDefinitionId =
                    owned.Result!.Data!.WorkflowDefinitionId,
                RequestReasonId = request.RequestReasonId,
                RequestCategory = request.RequestCategory,
                RequestReasonText = request.RequestReasonText,
                Description = request.Description,
        ...
        [AccessAction(
            "Submit",
            "Submit My Profile Change",
            Description = "Submit perubahan profil ke generic workflow",
            AccessType = AccessTypes.Update,
            SortOrder = 4)]
        [AccessPermission("MyProfileChange", "Submit")]
        public async Task<IActionResult> Submit(
            Guid id,
            [FromBody] EmployeeProfileChangeSelfServiceSubmitRequest? request,
            CancellationToken cancellationToken)
        {
            var owned = await GetOwnedAsync(
                id,
            ...
            var domainRequest = new EmployeeProfileChangeWorkflowSubmitRequest
            {
                Note = request?.Note,
            ...
            return ToActionResult(
                await _workflowIntegrationService.SubmitAsync(
                    id,
                    domainRequest,
                    owned.Context!.UserId,
                    cancellationToken));
        ...
        [AccessAction(
            "Cancel",
            "Cancel My Profile Change",
            Description = "Membatalkan atau menarik perubahan profil",
            AccessType = AccessTypes.Update,
            SortOrder = 5)]
            ...
            CancellationToken cancellationToken)
        {
            ...
            return ToActionResult(
                await _workflowIntegrationService.CancelAsync(
                    id,
                    request,
                    owned.Context!.UserId,
                    cancellationToken));
        ...
        [AccessAction(
            "Delete",
            "Delete My Profile Change",
            Description = "Menghapus draft perubahan profil milik sendiri",
        ...
        private async Task<(
            ...
            IActionResult? Error)> ResolveContextAsync(
                CancellationToken cancellationToken)
        {
            try
            {
                var context = await _humanResourceContextService.GetCurrentAsync(
                    cancellationToken);
        ...
        private async Task<(
            HumanResourceUserContextDto? Context,
            EmployeeProfileChangeServiceResult<EmployeeProfileChangeResponse>?
                Result,
            IActionResult? Error)> GetOwnedAsync(
                Guid id,
                bool requireRequester,
                CancellationToken cancellationToken)
        {
            var contextResult = await ResolveContextAsync(cancellationToken);
            if (contextResult.Error != null)
            ...
            var isSameWorkforce =
                result.Data.WorkforceProfileId == context.WorkforceProfileId;
            var isRequester = result.Data.RequestedByUserId == context.UserId;

            if (!isSameWorkforce || (requireRequester && !isRequester))
            {
                return (
                    context,
                    null,
                    NotFound(
                        ApiResponse<object>.Fail(
                            StatusCodes.Status404NotFound,
                            "Pengajuan perubahan profil tidak ditemukan atau tidak dapat diubah oleh user login.")));
            }
        ...
        {
            if (result.Data != null)
                SanitizeForSelfService(result.Data);

            return result.Success
                ? StatusCode(
                    result.StatusCode,
                    ApiResponse<EmployeeProfileChangeResponse>.Ok(
                        result.Data,
                        result.Message))
                : StatusCode(
                    result.StatusCode,
        ...
        private IActionResult ToActionResult(
            EmployeeProfileChangeServiceResult<
                EmployeeProfileChangeWorkflowResponse> result)
        {
            if (result.Data?.ProfileChange != null)
                SanitizeForSelfService(result.Data.ProfileChange);

            return result.Success
                ? StatusCode(
                    result.StatusCode,
                    ApiResponse<EmployeeProfileChangeWorkflowResponse>.Ok(
                        result.Data,
                        result.Message))
                : StatusCode(
                    result.StatusCode,
        ...
        private static void SanitizeForSelfService(
            EmployeeProfileChangeResponse response)
        {
            foreach (var verification in response.Verifications)
            {
                verification.EvidenceFilePath = null;
                verification.EvidenceFileName = null;
                verification.EvidenceDownloadUrl = null;
            }
        }
    }
}
